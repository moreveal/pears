...
new fdrawName[][] = // Название Вещи
{
	...
	"ЏEP–…ЏHA•_C–EC’", "A‡”M…H…†", "‚Oѓ„O ЊPOЊYCK", "Kњћoўa¦¦", "KEY CASE GRAVES", "ЌEPEЊ_OC…P…CA", "HEKPOHO–…KOH"
};
new fdrawNameEN[][] = // Название Вещи на Английском
{
	...
	"THERMITE MIXTURE", "ALUMINIUM", "GODJO PASS", "KILOWATT", "KEY CASE GRAVES", "OSIRIS SKULL", "NECRONOMICON"
};
...
new friskName[][] = // Название Вещи
{
	...
	"Термитная смесь", "Алюминий", "Пропуск Годжо", "Киловатт", "Ключ от Похоронного Кейса", "Череп Осириса", "Некрономикон" // 237-243
};
...
stock i_page(playerid, page) // Переключаем страницы главного раздела
{
	...
	if(Backpack[playerid] == 1) // Если рюкзак открыт
	{
		if(page == 0 && Page[playerid] != 0)
		{
			...
			Page[playerid] = 0;
			i_switches(playerid, 0, 1);
			for(new inva = 0; inva < 20; inva++) i_tile(playerid, PlayerInfo[playerid][pBackInven][inva], PlayerInfo[playerid][pBackInvenQuan][inva], inva, PlayerInfo[playerid][pBackInvenPara][inva], PlayerInfo[playerid][pBackInvenType][inva], PlayerInfo[playerid][pBackInvenPack][inva]);
		}
		else if(page == 1 && Page[playerid] != 1)
		{
			PlayerPlaySound(playerid,17803,0,0,0);
			Page[playerid] = 1;
			i_switches(playerid, 1, 1);
			for(new inva = 20; inva < 40; inva++) i_tile(playerid, PlayerInfo[playerid][pBackInven][inva], PlayerInfo[playerid][pBackInvenQuan][inva], inva, PlayerInfo[playerid][pBackInvenPara][inva], PlayerInfo[playerid][pBackInvenType][inva], PlayerInfo[playerid][pBackInvenPack][inva]);
		}
		else if(page == 2 && Page[playerid] != 2)
		{
			...
			for(new inva = 40; inva < 60; inva++) i_tile(playerid, PlayerInfo[playerid][pBackInven][inva], PlayerInfo[playerid][pBackInvenQuan][inva], inva, PlayerInfo[playerid][pBackInvenPara][inva], PlayerInfo[playerid][pBackInvenType][inva], PlayerInfo[playerid][pBackInvenPack][inva]);
		}
		else if(page == 3 && Page[playerid] != 3)
		{
			...
			for(new inva = 60; inva < 80; inva++) i_tile(playerid, PlayerInfo[playerid][pBackInven][inva], PlayerInfo[playerid][pBackInvenQuan][inva], inva, PlayerInfo[playerid][pBackInvenPara][inva], PlayerInfo[playerid][pBackInvenType][inva], PlayerInfo[playerid][pBackInvenPack][inva]);
		}
	}
	return 1;
}
...
stock put_thing_player(playerid, thingId, quan, para, qara, thingType, thingPack, slot)
{
	...
	if(OnlineInfo[playerid][oShowInterface] == 1) i_tile(playerid, PlayerInfo[playerid][pInven][slot], PlayerInfo[playerid][pInvenQuan][slot], slot, PlayerInfo[playerid][pInvenPara][slot], PlayerInfo[playerid][pInvenType][slot], PlayerInfo[playerid][pInvenPack][slot]), PlayerPlaySound(playerid,1052,0,0,0);
	return slot;
}
...
stock i_limit(playerid, thingId, &getQuan, &getLimit) // Проверяем лимиты инвентаря
{
	...
	lim[4] = 100*pow, lim[5] = 100*pow, lim[6] = 100*pow, lim[7] = 100*pow, lim[9] = 20, lim[18] = 100*pow, lim[20] = 20*pow,lim[22] = 10*pow, lim[27] = 100*pow, lim[28] = 100*pow, lim[29] = 100*pow, lim[30] = 100*pow;
...
stock ShowTabBackpack(playerid)
{
	...
	// Количество страниц в рюкзаке (СЮДА И МЕНЯЕМ КОЛИЧЕСТВО СТРАНИЦ В ЗАВИСИМОСТИ ОТ ПРОКАЧКИ РЮКЗАКА 1 - 4)
	Backpages[playerid] = 3;
...
/* Инфа для создания рюкзака:
- Есть сток для проверки, есть ли у игрока аксессуар рюкзака на спине HasABackpack(playerid)
- В object_custom.pwn лежит stock IsABackpack(m) туда записать все рюкзаки по моделям и давать этот pwn зверю при добавлении новых рюкзаков
*/
