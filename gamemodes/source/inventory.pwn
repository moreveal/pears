...
new fdrawName[][] = // Название Вещи
{
	...
	"WEED PACK", "COCAINE PACK", "KH…‚A AHAPX…CЏA", "CO‡•HA• K…C‡OЏA", "KAYCЏ…ЌECKA• COѓA", "X‡OP…CЏ‘† ‹OѓOPOѓ", "METH PACK", "DUMMY METH PACK"
};
...
new friskName[][] = // Название Вещи
{
	...
	"Упаковка травы", "Упаковка порошка", "Поваренная книга Анархиста", "Соляная кислота", "Каустическая сода", "Хлористый водород", "Упаковка синего порошка", "Упаковка некачественного синего порошка" // 271 - 278
};
...
stock TopThing(i)
{
	...
	|| i >= 274 && i <= 276 || i == 277 || i == 278) return true;
	return false;
}
...
new friskPick[] = // ID Модельки в Инвентаре (обычный предмет)
{
	...
	1578,1279,12494,12725,12726,12724,1579,1575 // 271 - 278
};
...
stock CheckThingQuan(t) // Имеет ли предмет количество (1 да, 0 нет)
{
	...
	|| t >= 271 && t <= 272 || t >= 274 && t <= 276 || t == 277 || t == 278 ? 1 : 0;
}
...
new friskDefault[] = // Гос. стоимости предметов
{
	...
	-1,-1,50000,1000,1000,1000,-1,-1 // 278
};
...
stock UnlegalThing(i, thingType, thingPack)
{
	if(thingType == 0 // Обычный предмет
		...
		&& (i == 263 || i == 264 || i == 271 || i == 272 || i == 277 || i == 278)) return 1;
	return 0;
}
