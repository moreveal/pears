...
#define INVENTER 197 // Максимальный ID (обычный предмет)
...
new fdrawName[][] = // Название Вещи
{
	...
	"PEM.KOMЊ‡EKЏ KAЏEPO‹","HAMA3‡‘K","P‘ЂKA", "KOPM ѓ‡• P‘ЂOK","ЊAKEЏ"
};
new fdrawNameEN[][] = // Название Вещи на Английском
{
	...
	"REPAIR KIT BOAT","NAMAZLYK","AQUA FISH", "FISH FOOD", "PACKAGE"
};
new friskName[][] = // Название Вещи
{
	...
	"Рем.комплект Катеров","Намазлык","Аквариумная рыбка", "Корм для рыбок", "Пакет" // 192 - 196
};
new friskPick[] = // ID Модельки в Инвентаре (обычный предмет)
{
	...
	19921,2833,1599,19561,1575 // 192 - 196
};
        ...
        else if(OnlineInfo[playerid][oInventSelectLeft] != 9999 && OnlineInfo[playerid][oInventSelectLeft] == inva) // Выполняем
		{
		    ...
		    if(thingType == 0) // Обычный предмет
		    {
				if(fpick == 1) return format(store, sizeof(store), "%d", inva), cmd_eatbread(playerid, store), i_resetveshi(playerid); // Хлеб
			 	...
			 	else if(fpick == 5) return cmd_usedrugs2(playerid), i_resetveshi(playerid); // Спиды
