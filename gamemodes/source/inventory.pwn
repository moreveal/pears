...
// 5. TopThing - топовые вещи для шкатулку
...
// 12. apartments_limit - лимит в квартире
// 13. Refrigerator_limit - лимит в холодильнике
// пиздец..

// Тип товара (0 обычный, 1 оружие, 2 аксессуар, 3 одежда, 4 мебель, 5 транспорт, 6 - животные)
// Упаковка (0 предмет, 1 подарок, 2 ящик, 3 Мешок, 4 Запечатанный Ящик, 5 шкатулка)
...
new fdrawName[][] = // Название Вещи
{
	...
	"COK AЊE‡’C…H.", "COK •Ђ‡OЌ.", "O‹OЉ…", "C…‚HA‡…?A‰…•", "C…‚HA‡…?A‰…•", "C…‚HA‡…?A‰…•", "ЏOЊ‡…‹O", "MOPO„EHOE", "KAPЏ‘_ЏAPO", "…3O‡EHЏA",
	...
	"PEM.KOMЊ‡EKЏ KAЏEPO‹","HAMA3‡‘K","P‘ЂKA", "KOPM ѓ‡• P‘ЂOK","ЊAKEЏ", "ЂA‡OHЌ…K C KPACKO†","KYK‡A_‹YѓY","OЂ‘ЌHA• C…‚HA‡…3A‰…•","Y‡YЌЋEHHA• C…‚HA‡…3A‰…•","MEЏA‡…ЌECKA• ЏPYЂA",
	"‹…‡KA", "KAPЏA OXOЏH…KA", "MEЋOK C ѓEH’‚AM…", "ЂOMЂA ‡…ЊYЌKA","3AЊЌACЏ…","AROSPEED","SkunkRacer","Venom","DC Sports",
	...
	"CK…PH","KO‡ЂACA","C…H…† ЊOPOЋOK", "Њ‡OXO† C…H…† ЊOPOЋOK", "ЏAЂ‡EЏK…", "Ѓ…‡’ЏP PECЊ…PAЏOPA", "OЂO‡OЌKA PECЊ…PAЏOPA", "PE3…HA", "OЂO‡OЌKA ЊPOЏ…‹O‚A3A", "Ѓ…‡’ЏP ЊPOЏ…‹O‚A3A",
	"WEED PACK", "COCAINE PACK", "KH…‚A AHAPX…CЏA", "CO‡•HA• K…C‡OЏA", "KAYCЏ…ЌECKA• COѓA", "X‡OP…CЏ‘† ‹OѓOPOѓ", "METH PACK", "DUMMY METH PACK", "RUBBER COMPOUND",
	"KOHCEP‹‘ ѓ‡• KOЏO‹","KOHCEP‹‘ ѓ‡• COЂAK","‡AKOMCЏ‹O ѓ‡• Њ…ЏOM‰E‹","‡EKAPCЏ‹O ѓ‡• Њ…ЏOM‰E‹"
};
new fdrawNameEN[][] = // Название Вещи на Английском
{
    "","BREAD","WEDDING RING","BOTTLE","WEED","OUIJA BOARD","MUSHROOMS","POWDER","AID KIT","CANISTER","EXPLOSIVES",
	...
	"ORANGE JUICE","APPLE JUICE","VEGETABLES","ALARM","ALARM","ALARM","FUEL", "ICE CREAM", "TAROT CARDS", "DUCT TAPE",
	...
	"REPAIR KIT BOAT","NAMAZLYK","AQUA FISH", "FISH FOOD", "PACKAGE", "CANS OF PAINT","VOODOO DOOL","IMPROVED ALARM SYSTEM","THE USUAL ALARM SYSTEM","METAL PIPE",
	"FORK","MAP HUNTER","BAG OF MONEY","STICKY BOMB","PARTS","AROSPEED","SkunkRacer","Venom","DC Sports",
	"ACT","Clutch Masters","Jacson Raceing","HPRacing","Ibach Springs","KONI suspension","BILSTEIN","YKOHAMA","TOYO","Falken Tire",
	"KVR Perfomance","Brembo","Wilwood", "Philin Customs", "JACK", "ARMORED FILM 1000", "ARMORED FILM 2000", "ARMORED FILM 3000", "BAG", "SUNSCREEN",
	"RADAR Detector", "RADAR Detector 2", "RADAR Jammer", "KEY CASE MANIAC", "KEY CASE YAKUZA", "Radio Interceptor",
	"THERMITE MIXTURE", "ALUMINIUM", "GODJO PASS", "KILOWATT", "KEY CASE GRAVES", "OSIRIS SKULL", "NECRONOMICON", "RABBIT FUR", "FOX FUR", "WOLF SKIN", "BEAR SKIN", "DEER SKIN", "PADS", "CAULDRON",
	"CAULDRON", "COUPON CLOTHES","COUPON ACCESSORY","COUPON CAR","COUPON CRYPTA","THREADS","WOOL","COMPACTED LEATHER","FABRIC","NEEDLES",
	"SKIRN","SAUSAGE","BLUE POWDER", "SHIT BLUE POWDER", "ECSTASY", "RESPIRATOR FILTER", "RESPIRATOR SHELL", "RUBBER", "MASK SHELL", "MASK FILTER",
	"WEED PACK", "COCAINE PACK", "ANARCHIST BOOK", "HYDROCHLORIC ACID", "CAUSTIC SODA", "HYDROGEN CHLORIDE", "METH PACK", "DUMMY METH PACK",
	"CANNED FOOD FOR CATS","CANNED FOOD FOR DOGS", "TREATS FOR PETS","MEDICINE FOR PETS"
};

new friskName[][] = // Название Вещи
{
    "","Хлеб","Золотое Кольцо","Бутылка","Трава","Доска Уиджи","Грибы","Порошок","Аптечка","Канистра","Пояс с Взрывчаткой", // 0 - 10
	...
	"Апельсиновый сок", "Яблочный сок", "Овощи", "Дом Сигнализация 1 ур.", "Дом Сигнализация 2 ур.", "Дом Сигнализация 3 ур.", "Топливо", "Мороженое", "Карты таро","Изолента", // 172 - 181
	"Деталь бомбы","Рем.комплект Авто","Краска для транспорта","Автомобильные диски","Гидравлика","Закись Азота","Деталь Тюнинга","шкатулка","Рем.комплект Мото","Рем.комплект Авиа", // 182 - 191
	"Рем.комплект Катеров","Намазлык","Аквариумная рыбка", "Корм для рыбок", "Пакет", "Баллончик с краской","Кукла вуду","Обычная Сигнализация","Улучшенная Сигнализация","Металическая Труба", // 192 - 201
	"Вилка","Карта Охотника","Мешок с деньгами","Бомба липучка", "Запчасти","AROSPEED","SkunkRacer","Venom","DC Sports", // 202 - 210
	...
	"Детектор радаров", "Улучшенный детектор радаров", "Глушилка радаров", "Ключ от шкатулки Маньяка", "Ключ от шкатулки Yakuza", "Радиоперехватчик", // 231 - 236
	"Термитная смесь", "Алюминий", "Пропуск Годжо", "Киловатт", "Ключ от Похоронного шкатулки", "Череп Осириса", "Некрономикон", "Мех Зайца", "Мех Лисы", "Шкура Волка", "Шкура Медведя", "Кожа Оленя","Прокладки","Маленький Котелок", // 237-250
	"Большой Котелок", "Купон одежда", "Купон аксессуар","Купон транспорт","Купон крипта","Нитки","Шерсть","Уплотненная Кожа","Ткань","Иголки", // 251 - 260
	"Скирк","Колбаса","Синий Порошок", "Некачественный Синий Порошок", "Таблетки", "Фильтр респиратора", "Оболочка респиратора", "Резина", "Оболочка противогаза", "Фильтр противогаза", // 261 - 270
	"Упаковка травы", "Упаковка порошка", "Поваренная книга Анархиста", "Соляная кислота", "Каустическая сода", "Хлористый водород", "Упаковка синего порошка", "Упаковка некачественного синего порошка", "Резиновая смесь", // 271 - 279
	"Консервы для кошек","Консервы для собак","Лакомство для питомцев","Лекарство для питомцев" // 280 - 283
};

// Топовые предметы, которым не надо выпадать так просто в шкатулки
stock TopThing(i)
{
	if(i == 10 || i == 11 || i == 26 || i == 42 || i == 49 || i >= 64 && i <= 67 || i == 94
	|| i == 176 || i == 177 || i == 210 || i == 213 || i == 214 || i == 215 || i == 216 || i == 219 || i == 220
	|| i == 222 || i == 223 || i == 224 || i == 239 || i == 241 || i == 242 || i == 263 || i == 267 || i >= 269 && i <= 272
	|| i >= 274 && i <= 276 || i == 277 || i == 278 || i == 279) return true;
	return false;
}

new friskPick[] = // ID Модельки в Инвентаре (обычный предмет)
{
    0,19579,2710,1520,12383,12345,1578,12378,11738,1650,19515, // 0 - 10
    ...
    19563,19564,19572,1614,1616,1622,3465,19568,12346,11747, // 172 - 181
	3082,19921,365,0,0,0,0,19918,19921,19921, // 182 - 191
	19921,2833,1599,19561,1575,365,12350,1614,1615,1135, // 192 - 201
	11715,19170,1550,363,19917,19917,19917,19917,19917, // 202 - 210
	...
	18875,364,330,11746,11746,2967,19921,19843,2684,12287, // 231 - 240
	12353,12341,12342,12387,12388,12385,12386,12384,12354,12374, // 241 - 250
	12375, 12446, 12447, 12445, 12451, 12464, 12465, 12466, 12467, 12468, // 251 - 260
	19569,12727,12382,12382,12381,12497,12498,12499,12495,12496, // 261 - 270
	1578,1279,12494,12725,12726,12724,1579,1575,1327, // 271 - 279
...
{
	return
	(t == 4 || t >= 6 && t <= 9 || t >= 18 && t <= 20 || t == 22 || t == 25 || t >= 27 && t <= 30 || t == 41 || t == 46 || t == 47 || t == 55 || t == 60 || t == 61
	|| t >= 64 && t <= 67 || t == 70 || t >= 71 && t <= 89 || t == 106 || t == 108 || t == 109 || t == 110 || t == 140 || t == 142 || t == 178
	|| t == 181 || t == 183 || t >= 190 && t <= 192 || t == 197 || t == 225 || t == 230 || t == 238 || t >= 252 && t <= 260 || t >= 263 && t <= 265) || t == 268
	|| t >= 271 && t <= 272 || t >= 274 && t <= 276 || t == 277 || t == 278 || t == 279 ? 1 : 0;
}

new friskDefault[] = // Гос. стоимости предметов
{
    -1,60,20000,-1,3000,-1,500,2000,3000,-1,-1, // 10
    -1,-1,300,100,-1,150,15000,10,200,200,3500, // 21
    700,200,1100,-1,15000,60,35,20,50,3000,700, // 32
    -1,-1,-1,-1,550,250,1500,2300,150,75000,3000, // 43
    900,900,500,1000,35000,350000,2000,-1, // 51
	...
	25,500,200,150,1500,5000,200,200,200,200, // 111
	...
	-1,900,-1,-1, 50000,100000,200000,400000,800000, // 210
	...
	30000,85000,120000,10000,10000,60000,-1,-1,1,40, // 240
	-1,-1,-1,-1,-1,-1,-1,-1,900, 5000, // 250
...
enum backpackEnum
{
	...
	backpackInvQara[MAX_INVEN_BACKPACK],
	backpackInvType[MAX_INVEN_BACKPACK],
	backpackInvPack[MAX_INVEN_BACKPACK]
}
new BackPackInfo[MAX_REALPLAYERS][backpackEnum];

new Backpack[MAX_REALPLAYERS]; // Открыт ли рюкзак в инвентаре
new Backpages[MAX_REALPLAYERS]; // Количество страниц рюкзака
...
stock CheckOnInventory(t)
{
  if(t == 12 || t == 15 || t >= 33 && t <= 36 || t == 43 || t == 51 || t >= 56 && t <= 59 || t == 63 || t == 68 || t == 69 || t >= 108 && t <=111 || t >=128 && t<= 139 ||
  t >= 143 && t <= 162) return 0;
  return 1;
}

CMD:gthinginfo(playerid)
{
	if(PlayerInfo[playerid][pSoska] < 20) return ErrorMessage(playerid, "{FF6347}Только для Системных администраторов");
	DP[5][playerid] = 1;
    gthinginfoPage(playerid);
    return 1;
}

stock gthinginfoPage(playerid)
{
...
CMD:gthingunix(playerid, const params[]) // Выдать предмет (НЕ ИСПОЛЬЗОВАТЬ эту команду без надобности, она не учитывает кучу условий для разных предметов)
{
	...
	//new unix = gettime();
    // Тип товара (0 обычный, 1 оружие, 2 аксессуар, 3 одежда, 4 мебель, 5 транспорт)
    new put_inva = GiveThingPlayer(params[0], params[1], params[2], params[3], 0, 0, 0, 9999);
    if(put_inva == -1) return ErrorMessage(playerid, "{FF6347}У игрока нет места в инвентаре");
...
stock tile_second(playerid, invatab) // Клацаем по ячейкам в правом разделе
{
	...
	else if(Tabs_Load[playerid] == 15) // Квартиры тумба
	{
		tab = OnlineInfo[playerid][oShowTabs];
		fpick = ApartmentsRoom[tab][aprInvent][inva];
		thingQuan = ApartmentsRoom[tab][aprInv][inva];
		fpara = ApartmentsRoom[tab][aprInvPara][inva];
		thingType = ApartmentsRoom[tab][aprInvType][inva];
		thingPack = ApartmentsRoom[tab][aprInvPack][inva];
	}
	else if(Tabs_Load[playerid] == 16) // Холодильник
	{
		tab = OnlineInfo[playerid][oShowTabs];
		fpick = Refrigerator[tab][fridgeInvent][inva];
		thingQuan = Refrigerator[tab][fridgeInv][inva];
		fpara = Refrigerator[tab][fridgeInvPara][inva];
		thingType = Refrigerator[tab][fridgeInvType][inva];
		thingPack = Refrigerator[tab][fridgeInvPack][inva];
	}
	...
	if(fpick == 0) // Если клацаем по пустой ячейке
	{
		if(OnlineInfo[playerid][oInventSelectLeft] == 9999) // Если ячейка в левом разделе не выбрана
		{
			if(OnlineInfo[playerid][oInventSelectRight] != 9999 && OnlineInfo[playerid][oInventSelectRight] != inva) // Перекладываем
			{
				...
				else if(Tabs_Load[playerid] == 15) shift_ApartmentsTable(playerid, tab, OnlineInfo[playerid][oInventSelectRight], inva);
				else if(Tabs_Load[playerid] == 16) shift_Refrigerator(playerid, tab, OnlineInfo[playerid][oInventSelectRight], inva);
		...
		else if(OnlineInfo[playerid][oInventSelectLeft] != 9999) // Кладём предмет из инвентаря в другой
		{
		    ...
		    if(Tabs_Load[playerid] == 6) 
			{
				if(NotGiveThing(myfpick, myThingType, PlayerInfo[playerid][pInvenQuan][myinva], myThingPack) && myThingType != 5) return ErrorMessage(playerid, "{FF6347}Этот предмет нельзя передавать, продавать или убирать"), i_resetveshi(playerid);
			}
			else
			{
			...
			else if(Tabs_Load[playerid] == 15) // Квартирная Тумба
			{
			    if(myThingType == 0 && myThingPack == 0)
			    {
			        if(CheckThingQuan(myfpick) == 1)
			        {
			            OnlineInfo[playerid][oInventSelectRight] = inva;
						format(string,sizeof(string),"{cccccc}Чтобы положить в тумбу {ff9000}%s {cccccc}введите количество\nНе меньше 1 и не больше 1.000.000",GetNameThing(1, myfpick, myThingType, myThingPack));
						ShowDialog(playerid,APARTMENTS_DIALOG_TABLEPUT,DIALOG_STYLE_INPUT,"{ff9000}Инвентарь",string,"Принять","Отмена");
						return 1;
			        }
			    }
			    put_ApartmentsTable(playerid, myinva, tab, myfpick, PlayerInfo[playerid][pInvenQuan][myinva], inva, myThingType, myThingPack);
			}
			else if(Tabs_Load[playerid] == 16) // Холодильник
			{
			    if(myThingType == 0 && myThingPack == 0)
			    {
			        ...
			        {
			            OnlineInfo[playerid][oInventSelectRight] = inva;
						format(string,sizeof(string),"{cccccc}Чтобы положить в холодильник {ff9000}%s {cccccc}введите количество\nНе меньше 1 и не больше 1.000.000",GetNameThing(1, myfpick, myThingType, myThingPack));
						ShowDialog(playerid,FRIDGE_DIALOG_PUT,DIALOG_STYLE_INPUT,"{ff9000}Инвентарь",string,"Принять","Отмена");
						return 1;
			        }
			    }
			    put_Refrigerator(playerid, myinva, tab, myfpick, PlayerInfo[playerid][pInvenQuan][myinva], inva, myThingType, myThingPack);
			}
	...
	else if(fpick > 0) // Что-то Лежит
	{
		...
		else if(OnlineInfo[playerid][oInventSelectRight] == inva) // Выполняем
		{
			...
			else if(Tabs_Load[playerid] == 15) return use_Apartments_table(playerid, tab, inva, 9999);
			else if(Tabs_Load[playerid] == 16) return use_Refrigerator(playerid, tab, inva, 9999);
		}
		else if(OnlineInfo[playerid][oInventSelectRight] != 9999 && OnlineInfo[playerid][oInventSelectRight] != inva)
		{
			...
			else if(Tabs_Load[playerid] == 15) tabfpick = ApartmentsRoom[tab][aprInvent][OnlineInfo[playerid][oInventSelectRight]], tabType = ApartmentsRoom[tab][aprInvType][OnlineInfo[playerid][oInventSelectRight]], tabPack = ApartmentsRoom[tab][aprInvType][OnlineInfo[playerid][oInventSelectRight]]; // Квартирная Тумба
			else if(Tabs_Load[playerid] == 16) tabfpick = Refrigerator[tab][fridgeInvent][OnlineInfo[playerid][oInventSelectRight]], tabType = Refrigerator[tab][fridgeInvType][OnlineInfo[playerid][oInventSelectRight]], tabPack = Refrigerator[tab][fridgeInvType][OnlineInfo[playerid][oInventSelectRight]]; // Холодильник
			...
			if(tabfpick > 0 && fpick == tabfpick)
			{
			    if(tabType == 0 && tabPack == 0) // Только обычные предметы
			    {
			    	if(CheckThingQuan(fpick) == 1) // Если количественные, складываем их в одну ячейку
					{
						...
						else if(Tabs_Load[playerid] == 15) return mix_ApartmentsTable(playerid, tab, OnlineInfo[playerid][oInventSelectRight], inva);
						else if(Tabs_Load[playerid] == 16) return mix_Refrigerator(playerid, tab, OnlineInfo[playerid][oInventSelectRight], inva);
					}
				}
			}
...
stock PreviouslySellThingPlayer(playerid, giveplayerid, const inputtext[])
{
	...
	if(NotGiveThing(fpick, thingType, PlayerInfo[playerid][pInvenQuan][inva], thingPack) && thingType != 5) return ErrorMessage(playerid, "{FF6347}Этот предмет нельзя продать этому игроку");
...
stock give_invent(playerid, giveplayerid, fpick, fquan, thingType, thingPack, inva)
{
	...
	if(NotGiveThing(fpick, thingType, PlayerInfo[playerid][pInvenQuan][inva], thingPack) && thingType != 5) return ErrorMessage(playerid, "{FF6347}Этот предмет нельзя передать этому игроку");
	if(NotGiveInflatabelBoat(playerid, fpick, thingType)) return i_resetveshi(playerid);
...
stock i_infofpick(playerid, fpick, thingQuan, inva, sels, fpara, thingType, thingPack) // Отображение выбранного предмета
{
	...
	{
	    if(PlayerInfo[playerid][pDrawLanguage] == false && Device[playerid] != 1) format(string, sizeof(string), "ЋKAЏY‡KA");
...
{
	ResetSwitchInInventory(playerid); // Выключаем и обновляем выбранную до этого кнопку переключатель

	if(Backpack[playerid] == 0) // Если рюкзак закрыт
	{
	...
	if(Backpack[playerid] == 1) // Если рюкзак открыт
	{
		if(page == 0 && Page[playerid] != 0)
		{
			PlayerPlaySound(playerid,17803,0,0,0);
		...
		else if(page == 1 && Page[playerid] != 1)
		{
			...
			Page[playerid] = 1;
			i_switches(playerid, 1, 1);
			for(new inva = 20; inva < 40; inva++) i_tile(playerid, BackPackInfo[playerid][backpackInvent][inva], BackPackInfo[playerid][backpackInv][inva], inva, BackPackInfo[playerid][backpackInvPara][inva], BackPackInfo[playerid][backpackInvType][inva], BackPackInfo[playerid][backpackInvPack][inva]);
		}
		else if(page == 2 && Page[playerid] != 2)
		{
			PlayerPlaySound(playerid,17803,0,0,0);
			Page[playerid] = 2;
			i_switches(playerid, 12, 1);
			for(new inva = 40; inva < 60; inva++) i_tile(playerid, BackPackInfo[playerid][backpackInvent][inva], BackPackInfo[playerid][backpackInv][inva], inva, BackPackInfo[playerid][backpackInvPara][inva], BackPackInfo[playerid][backpackInvType][inva], BackPackInfo[playerid][backpackInvPack][inva]);
		}
		else if(page == 3 && Page[playerid] != 3)
		{
			PlayerPlaySound(playerid,17803,0,0,0);
			Page[playerid] = 3;
			i_switches(playerid, 13, 1);
			for(new inva = 60; inva < 80; inva++) i_tile(playerid, BackPackInfo[playerid][backpackInvent][inva], BackPackInfo[playerid][backpackInv][inva], inva, BackPackInfo[playerid][backpackInvPara][inva], BackPackInfo[playerid][backpackInvType][inva], BackPackInfo[playerid][backpackInvPack][inva]);
		}
	}
	return 1;
}

stock ResetSwitchInInventory(playerid)
{
	if(Page[playerid] == 0) i_switches(playerid, 0, 0);
	else if(Page[playerid] == 1) i_switches(playerid, 1, 0);
	else if(Page[playerid] == 2) i_switches(playerid, 12, 0);
	else if(Page[playerid] == 3) i_switches(playerid, 13, 0);
	return true;
}

// Закрываем переключатели
stock CloseSwitchInventory(playerid)
{
	PlayerTextDrawHide(playerid, PlaNestTabs[0][playerid]);
	PlayerTextDrawHide(playerid, PlaNestTabs[1][playerid]);
	PlayerTextDrawHide(playerid, PlaNestTabs[12][playerid]);
	PlayerTextDrawHide(playerid, PlaNestTabs[13][playerid]);
	return true;
}

stock OffSwitchInventory(playerid)
{
	close_i_switches(playerid, 0);
	close_i_switches(playerid, 1);
	close_i_switches(playerid, 12);
	close_i_switches(playerid, 13);
	return true;
}
...
stock i_resetveshi(p) // Сброс отображения выбранной ячейки основного раздела
{
	...
	{
		if(Page[p] == 0 && OnlineInfo[p][oInventSelectLeft] <= 19) PlayerTextDrawBackgroundColour(p, PlaNestPick[OnlineInfo[p][oInventSelectLeft]][p], PlayerInfo[p][pStyle1]),PlayerTextDrawShow(p, PlaNestPick[OnlineInfo[p][oInventSelectLeft]][p]);
		else if(Page[p] == 1 && OnlineInfo[p][oInventSelectLeft] >= 20
			&& OnlineInfo[p][oInventSelectLeft] <= 39) PlayerTextDrawBackgroundColour(p, PlaNestPick[OnlineInfo[p][oInventSelectLeft]-20][p], PlayerInfo[p][pStyle1]),PlayerTextDrawShow(p, PlaNestPick[OnlineInfo[p][oInventSelectLeft]-20][p]);
		else if(Page[p] == 2 && OnlineInfo[p][oInventSelectLeft] >= 40
			&& OnlineInfo[p][oInventSelectLeft] >= 59) PlayerTextDrawBackgroundColour(p, PlaNestPick[OnlineInfo[p][oInventSelectLeft]-40][p], PlayerInfo[p][pStyle1]),PlayerTextDrawShow(p, PlaNestPick[OnlineInfo[p][oInventSelectLeft]-40][p]);
		else if(Page[p] == 3 && OnlineInfo[p][oInventSelectLeft] >= 60
...
{
	if(Page[playerid] == 0 && cell <= 19 
	|| Page[playerid] == 1 && cell >= 20 && cell <= 39
	|| Page[playerid] == 2 && cell >= 40 && cell <= 59
	|| Page[playerid] == 3 && cell >= 60 && cell <= 79)
	{
		new cell2 = cell;
		if(Page[playerid] == 1) cell = cell-20;
		else if(Page[playerid] == 2) cell = cell-40;
		else if(Page[playerid] == 3) cell = cell-60;
...
stock GetModelPickItem(playerid, thingId, thingType, thingPara, thingPack, sels)
{
	...
	else if(IsACasePackID(thingPack)) yesFindModel = GetModelCustomCase(thingPack); // шкатулка
...
stock get_invent(playerid, thingId, thingType) // Поиск предмета в инвентаре (Основные страницы + Торговля + рюкзак)
{
	new quan = 0;
	for(new i = 0; i < 40; i++)
	{
	...
	if(HasABustAks(playerid,1) != -1)
	{
		...
		{
			if(BackPackInfo[playerid][backpackInvent][i] == thingId && BackPackInfo[playerid][backpackInv][i] > 0 && BackPackInfo[playerid][backpackInvType][i] == thingType) quan += BackPackInfo[playerid][backpackInv][i];
		}
	}
	return quan;
}
...
stock get_invent2(playerid, stat, thingType) // Поиск предмета в инвентаре (Только две страницы основного инвентаря + рюкзак)
{
	...
	if(HasABustAks(playerid,1) != -1)
	{
		for(new i = 0; i < MAX_INVEN_BACKPACK; i++)
		{
			if(BackPackInfo[playerid][backpackInvent][i] == stat && BackPackInfo[playerid][backpackInv][i] > 0 && BackPackInfo[playerid][backpackInvType][i] == thingType) quan += BackPackInfo[playerid][backpackInv][i];
		}
	}
	return quan;
}

stock get_invent2_Slot(playerid, thingId, thingType, bool: positiveQuan = true) // Поиск предмета в инвентаре (Только две страницы основного инвентаря) и возвращаем slot
{
	new slot = -1;
	for(new i = 0; i < 40; i++)
	{
		if(PlayerInfo[playerid][pInven][i] == thingId && PlayerInfo[playerid][pInvenType][i] == thingType && (!positiveQuan || PlayerInfo[playerid][pInvenQuan][i] > 0)) 
		{
			slot = i;
			break;
		}
...
stock GiveThingPlayer(playerid, thingId, quan, para, qara, thingType, thingPack, useinva = 9999) // Даём игроку предмет в инвентарь
{
	...
	if(!Backpack[playerid])
	{
		...
		if(useinva == 9999 || useinva == -1) // Не знаем в какую ячейку класть
		{
			if(thingType == 0 && thingPack == 0) // Обычный предмет
			{
				...
				else // Объект не имеет количество
				{
					for(new i = 0; i < 40; i++)
					{
						if(PlayerInfo[playerid][pInven][i] == 0) // Ищем пустую ячейку
	...
	if(inva == -1)
	{
		...
		if(IsABackPack(thingId) && thingPack == 0)
		{
			i_resetveshi(playerid);
			ErrorMessage(playerid,"{ff6347}Рюкзак нельзя убирать в рюкзак!");
			return -1;
		}
		if(useinva == 9999 || useinva == -1) // Не знаем в какую ячейку класть
		{
			if(thingType == 0 && thingPack == 0) // Обычный предмет
			{
				if(CheckThingQuan(thingId) == 1) // Предмет имеет количество (Складывается в одну ячейку)
				{
					...
					{
						if(BackPackInfo[playerid][backpackInvent][i] == thingId && BackPackInfo[playerid][backpackInvType][i] == thingType && BackPackInfo[playerid][backpackInvPack][i] == thingPack) // Ищем тот, где уже предмет лежит
						{
							inva = i;
							put_thing_player_backpack(playerid, thingId, quan, para, 0, thingType, thingPack, i); // qara 0 - поскольку количественные предметы не могут иметь статус краденного
							find = 1;
							break;
						}
					}
					if(find == 0) // Если не нашли, ищем пустую
					{
						for(new i = 0; i < slots; i++)
						{
							if(BackPackInfo[playerid][backpackInvent][i] == 0) // Ищем пустую ячейку
							{
								inva = i;
								put_thing_player_backpack(playerid, thingId, quan, para, 0, thingType, thingPack, i); // qara 0 - поскольку количественные предметы не могут иметь статус краденного
								break;
				...
				else // Объект не имеет количество
				{
					for(new i = 0; i < slots; i++)
					{
						...
						{
							inva = i;
							put_thing_player_backpack(playerid, thingId, quan, para, qara, thingType, thingPack, i);
							break;
						}
					}
				}
...
stock ThingParameters(playerid, thingId, &quan, &para)
{
	...
	else if(thingId == 233 && para == 0) quan = GetFullThingQuan(thingId), para = unix+RADAR_JAMMER_SOFTWARE_TIME*24*60*60; // Глушилка радаров
	else if(thingId == 236 && para == 0) quan = GetFullThingQuan(thingId), para = unix+RADIO_INTERCEPTOR_SOFTWARE_TIME*24*60*60; // Радиоперехватчик
...
stock put_thing_player(playerid, thingId, quan, para, qara, thingType, thingPack, slot)
{
	...
	{
	    if(PlayerInfo[playerid][pInvenPara][slot] > 0)
		{
			if(PlayerInfo[playerid][pInvenPara][slot] > para) PlayerInfo[playerid][pInvenPara][slot] = para;
		}
		else PlayerInfo[playerid][pInvenPara][slot] = para;
	}
	else PlayerInfo[playerid][pInvenPara][slot] = para;
	
	PlayerInfo[playerid][pInvenQara][slot] = qara; // Статус краденного предмета
	PlayerInfo[playerid][pInvenType][slot] = thingType; // Тип предмета
	PlayerInfo[playerid][pInvenPack][slot] = thingPack; // Упаковка предмета
	
	SaveInvent(playerid, slot); // Сохраняем ячейку сразу в инвентаре
	if(OnlineInfo[playerid][oShowInterface] == 1 && !Backpack[playerid]) i_tile(playerid, PlayerInfo[playerid][pInven][slot], PlayerInfo[playerid][pInvenQuan][slot], slot, PlayerInfo[playerid][pInvenPara][slot], PlayerInfo[playerid][pInvenType][slot], PlayerInfo[playerid][pInvenPack][slot]), PlayerPlaySound(playerid,1052,0,0,0);
	return slot;
}
	...
	for(new i = 0; i < MAX_INVEN; i++)
	{
		...
		if(is_null == false)
		{
			...
			if (JSON_Parse(string_json, node) == JSON_CALL_NO_ERR) 
			{
				...
				if(PlayerInfo[playerid][pInvenQuan][i] < 0) PlayerInfo[playerid][pInven][i] = 0;
				//new slotLol[512];
	...
	for(new i = 0; i < MAX_MARK; i++)
	{
		...
		if(is_null == false)
		{
			...
			if (JSON_Parse(string_json, node) == JSON_CALL_NO_ERR) 
			{
				...
				if(PlayerInfo[playerid][pMarkInvenQuan][i] < 0) PlayerInfo[playerid][pMarkInven][i] = 0;
			}
		}
	}
	return 1;
...
stock GetNameThing(readStatus, thingId, thingType, thingPack) // Получаем обычное имя предмета по его типу (Для текста, диалоговых окон и логов)
{
	...
	else if(thingPack >= 1) // 0 предмет, 1 подарок, 2 ящик, 3 Мешок, 4 Запечатанный ящик, 5 шкатулка (помещается только 1 предмет и занимает 1 ячейку)
	{
	    new hideName[30];
...
stock JustOneThingInventory(i, type) // Проверка на наличие предметов, которые не могут повторяться в инвентаре (Только 1)
{
	...
	|| i == 42 || i == 43 || i == 48 || i == 51 || i == 63 || i == 91 || i >= 231 && i <= 233)
...
{
	if (IsACasePackID(thingPack)) return 0; // шкатулку можно выбрасывать и передавать всегда

	if(type == 0 && (i == 10 || i == 12 || i == 17 || i == 43 || i == 51 || i == 63 || i == 250 || i == 251 || i == 156 && quan == 2)
		|| type == 1 && (i == 34)
		|| type == 5) return 1;
	return 0;
}
...
{
	if(thingType == 0 // Обычный предмет
		&& !IsACasePackID(thingPack) // Не шкатулка
		&& (i == 4 || i == 6 || i == 7 || i == 10 || i == 11 || i == 12 || i == 140 || i == 19 || i == 90 || i == 182 || i == 205 || Hank_IsGood(i))
		&& (i == 263 || i == 264 || i == 271 || i == 272 || i == 277 || i == 278 || i == 279)) return 1;
	return 0;
}
...
{
	if(IsACasePackID(thingPack) // Предмет упакован в шкатулка
		&& PlayerInfo[playerid][pLevel] <= 3) return ErrorMessage(playerid, "{FF6347}Вы не можете передать, выбросить или продать шкатулку\n{ffcc66}Передача шкатулок будет доступна с 4 уровня [ /stats ]");
	return 0;
}
...
stock i_limit(playerid, thingId, &getQuan, &getLimit) // Проверяем лимиты инвентаря
{
	...
	lim[4] = 100*pow, lim[5] = 100*pow, lim[6] = 100*pow, lim[9] = 20, lim[18] = 100*pow, lim[20] = 20*pow,lim[22] = 10*pow;
	...
	lim[268] = 50*pow; // Резина
	lim[252] = 100, lim[253] = 100, lim[254] = 100, lim[255] = 100; // Купоны
	lim[7] = lim[263] = lim[264] = lim[265] = 100*pow; // Порошки
	lim[256] = 300, lim[257] = 300, lim[258] = 300, lim[259] = 300, lim[260] = 300; // Наборы для крафта
	lim[271] = lim[272] = lim[277] = lim[278] = 20; // Упаковки веществ
	lim[279] = 50*pow; // Резиновая смесь
	for (new i = 274; i <= 276; i++) lim[i] = 10; // Ингредиенты для нарко
...
stock apartments_limit(pt, thingId, &getQuan, &getLimit) // Проверяем лимиты тумбы в квартире
{
	new lim[sizeof(friskName)];
	for(new i = 0; i < sizeof(friskName); i++) lim[i] = 1;
	lim[8] = 100, lim[19] = 1000, lim[41] = 1000, lim[25] = 999000000; // Аптечки, Отмычки, Бенгальские Свечи, Деньги 999кк
	lim[4] = 100000, lim[5] = 100000, lim[6] = 100000, lim[9] = 20, lim[18] = 10000, lim[20] = 10000, lim[27] = 50000, lim[28] = 50000, lim[29] = 50000, lim[30] = 50000;
	...
	lim[238] = 1000; // Алюминий
    lim[268] = 500;  // Резина
	lim[252] = 100, lim[253] = 100, lim[254] = 100, lim[255] = 100; // Купоны
	lim[7] = lim[263] = lim[264] = lim[265] = 100000; // Порошки
	lim[256] = 1000, lim[257] = 1000, lim[258] = 1000, lim[259] = 1000, lim[260] = 1000; // Наборы для крафта
	lim[271] = lim[272] = lim[277] = lim[278] = 5000; // Упаковки веществ
	lim[279] = 500; // Резиновая смесь
	for (new i = 274; i <= 276; i++) lim[i] = 100; // Ингредиенты для нарко
...
stock Refrigerator_limit(pt, thingId, &getQuan, &getLimit) // Проверяем лимиты холодильника
{
	new lim[sizeof(friskName)];
	for(new i = 0; i < sizeof(friskName); i++) lim[i] = 1;
	lim[8] = 100, lim[19] = 1000, lim[41] = 1000, lim[25] = 999000000; // Аптечки, Отмычки, Бенгальские Свечи, Деньги 999кк
    ...
    getQuan = get_Refrigerator(pt, thingId, 0);
    getLimit = lim[thingId];
	return 1;
}

stock IsFullQuanThingid(playerid, thingId, thingQuan, thingType)
{
	if(CheckThingQuan(thingId) == 1 && thingType == 0)
	{
		new getQuan, getLimit;
...
stock use_sklad(playerid, wh, inva, useinva)
{
	...
	else if(thingType == 0)
	{
	    if(CheckThingQuan(fpick) == 1) // Предмет имеет количество
		{
			...
			else if(Drugs_IsPackage(fpick) || (fpick >= 4 && fpick <= 7)) maxQuan = OrganInfo[wh][gUnitStat][22], maxTime = PlayerInfo[playerid][pGacc][8]+OrganInfo[wh][gUnitStat][23]*60;
...
stock sklad_limit(thingId, thingType) // Проверяем лимиты склада организации
{
	...
	if(thingType == 0) // Обычные Предметы
	{
		...
		else if(Drugs_IsPackage(thingId)) getLimit = 5000; // Упаковки веществ
...
stock player_tile(playerid, inva)
{
	...
	if(fpick == 0) // Ничего нет
	{
		if(OnlineInfo[playerid][oInventSelectRight] == 9999)
		{
			...
			else if(OnlineInfo[playerid][oInventSelectBackPack] != 9999) // Перекладываем
			{
				...
				PlayerInfo[playerid][pInven][inva] = BackPackInfo[playerid][backpackInvent][OnlineInfo[playerid][oInventSelectBackPack]];
				PlayerInfo[playerid][pInvenQuan][inva] = BackPackInfo[playerid][backpackInv][OnlineInfo[playerid][oInventSelectBackPack]];
				PlayerInfo[playerid][pInvenPara][inva] = BackPackInfo[playerid][backpackInvPara][OnlineInfo[playerid][oInventSelectBackPack]];
				PlayerInfo[playerid][pInvenQara][inva] = BackPackInfo[playerid][backpackInvQara][OnlineInfo[playerid][oInventSelectBackPack]];
				PlayerInfo[playerid][pInvenType][inva] = BackPackInfo[playerid][backpackInvType][OnlineInfo[playerid][oInventSelectBackPack]];
				PlayerInfo[playerid][pInvenPack][inva] = BackPackInfo[playerid][backpackInvPack][OnlineInfo[playerid][oInventSelectBackPack]];
				i_tile(playerid, PlayerInfo[playerid][pInven][inva], PlayerInfo[playerid][pInvenQuan][inva], inva, PlayerInfo[playerid][pInvenPara][inva], PlayerInfo[playerid][pInvenType][inva], PlayerInfo[playerid][pInvenPack][inva]);
		...
		else if(OnlineInfo[playerid][oInventSelectRight] != 9999) // Берём Откуда-то
		{
			...
			{
				if(OnlineInfo[playerid][oShowTabs] != 9999) return use_Apartments_table(playerid, OnlineInfo[playerid][oShowTabs], OnlineInfo[playerid][oInventSelectRight], OnlineInfo[playerid][oInventSelectLeft]);
			}
			else if(Tabs_Load[playerid] == 16)
			{
				if(OnlineInfo[playerid][oShowTabs] != 9999) return use_Refrigerator(playerid, OnlineInfo[playerid][oShowTabs], OnlineInfo[playerid][oInventSelectRight], OnlineInfo[playerid][oInventSelectLeft]);
			}
			else i_resettabs(playerid);
		}
	}
	else if(fpick > 0) // Что-то Лежит
	{
		...
		{
			UseItem(playerid,inva,fpick,fquan,fpara,thingType,thingPack);
			OnlineInfo[playerid][oInventSelectLeft] = 9999;
		}
	}
	return 1;
...
stock ShowMessageErrorGun(playerid)
{
	...
	\n{ffcc66}У вашего персонажа временно изъято оружие из рук.\
...
stock SkupshikListItem(i, t, p)
{
	...
	if(i >= 1 && i <= 3 || i == 13 || i == 14 || i >= 16 && i <= 20 || i >= 22 && i <= 24 || i == 31 || i == 32 || i >= 37 && i <= 42 || i >= 44 && i <= 50 
...
stock UseItem(playerid,inva, fpick,fquan,fpara,thingType,thingPack)
{
	// Упаковываем подарок
	if(Hold[playerid] == 4)
	{
	...
	// Распаковываем подарок или шкатулку
	...
	if(thingType == 0) // Обычный предмет
	{
		...
		else if(fpick == 4) return pc_cmd_usedrugs2(playerid), i_resetveshi(playerid); // Трава
		...
		else if (fpick == 263) return UseDrugs(playerid, DRUGS_TYPE_BLUE_COCAINE, inva), i_resetveshi(playerid); // Синий порошок
		else if (fpick == 264) return UseDrugs(playerid, DRUGS_TYPE_DUMMY_BLUE_COCAINE, inva), i_resetveshi(playerid); // Некачественный синий порошок
		else if (fpick == 265) return UseDrugs(playerid, DRUGS_TYPE_ECSTASY, inva), i_resetveshi(playerid); // Таблетки
		...
		*/
		...
		fpick == 127 || fpick == 141 || fpick == 163 || fpick == 167 || fpick == 261 || fpick == 262) return drink_eat(playerid, inva, fpick, thingPack), i_resetveshi(playerid); // Алкашка / Еда
		...
		else if(fpick >= 145 && fpick <= 155)
		{
		...
		else if(fpick >= 244 && fpick <= 248) return ShowDialog(playerid,1700,DIALOG_STYLE_MSGBOX,"{ffcc00}*","{ffcc66}Шкура и мех используются для крафтов предметов с особыми бонусами! \n\n{444444}Подробнее можно узнать на швейном станке(Продается в IKEA для домов)","*","");
		else if(fpick == 205) return UseStickyBomb(playerid, inva);
		else if(fpick == 203) return huntermap(playerid);
		else if (fpick >= 231 && fpick <= 233) return pc_cmd_thing(playerid), i_resetveshi(playerid);
		else if (Drugs_IsPackage(fpick)) {
			new drugid, drugquan;
			Drugs_GetPackageAmount(fpick, drugid, drugquan);
			return TakeInvent(playerid, fpick, 1, 0, inva), GiveThingPlayer(playerid, drugid, drugquan, 0, 0, 0, 0), i_resetveshi(playerid);
		}
		else if (fpick >= 274 && fpick <= 276) {
			if (!NarcoSpotPlayer_IsInside(playerid, NARCO_SPOT_LABORATORY)) return ErrorMessage(playerid, "{FF6347}Вы должны находиться в лаборатории");
			i_resetveshi(playerid), CloseFrisk(playerid), CancelSelectTextDraw(playerid);
			if (Hold[playerid] == 0 || Hold[playerid] == 18)
			{
				if (NarcoSpotGetIngredientType(fpick) == _:NarcoSpotPlayerInfo[playerid][nspIngredient])
				{
					NarcoSpotPlayerInfo[playerid][nspIngredient] = NARCO_INGREDIENT_NONE;
					Hold[playerid] = 0;
					RemovePlayerAttachedObject(playerid, 1);
				} else {
					NarcoSpotPlayerInfo[playerid][nspIngredient] = e_NarcoSpotIngredient: NarcoSpotGetIngredientType(fpick);
					switch (NarcoSpotPlayerInfo[playerid][nspIngredient])
					{
						case NARCO_INGREDIENT_HYDROCHLORIC_ACID: 	SetPlayerAttachedObject(playerid, 1, 12725, 6, 0.142000, 0.050000, -0.012000, 8.300001, -0.300000, -110.499977, 0.619999, 0.661000, 0.709000, 0x0, 0x0);
						case NARCO_INGREDIENT_CAUSTIC_SODA: 		SetPlayerAttachedObject(playerid, 1, 12726, 6, 0.162999, 0.031000, 0.077999, 13.000000, -21.599962, 93.199836, 0.305999, 0.390000, 0.301000, 0x0, 0x0);
						case NARCO_INGREDIENT_HYDROGEN_CHLORIDE: 	SetPlayerAttachedObject(playerid, 1, 12724, 6, 0.063999, 0.044999, 0.082000, 5.699995, -6.399995, 128.299865, 0.449001, 0.417000, 0.435999, 0x0, 0x0);
						default: return 1;
					}
					Hold[playerid] = 18;
					FORMAT:buf("{99ff66}Вы взяли: %s", GetNameThing(0, fpick, 0, 0));
					SuccessMessage(playerid, buf);
				}
			...
			new placeid = NarcoSpotPlayer_GetNearPlaceId(playerid, 1.5, .only_owner = true);
			if (placeid == NARCO_SPOT_INVALID_ID) return ErrorMessage(playerid, "{FF6347}Нужно находиться рядом с местом, где ведётся приготовление порошка");

			new spotid = NarcoSpotPlayer_GetId(playerid);
			new ingredientid = NarcoPlaceInfo[spotid][placeid][npdWaterings];
			if (ingredientid < 0 || ingredientid >= 3) return 0; // Несуществующий ингредиент (на всякий случай)

			SetPlayerChatBubble(playerid, "смотрит нужный ингредиент в книге", COLOR_PURPLE, 20.0, 1500);
			new ingredient = NarcoPlaceInfo[spotid][placeid][npdIngredients][ingredientid];
			FORMAT:message("{cccccc}[ Место №%d ]\n\nСледующий ингредиент: {99ff66}%s", placeid + 1, GetNameThing(0, NarcoSpotGetIngredientThing(ingredient), 0, 0));
			return SuccessMessage(playerid, message);
		}
		...
		else if(fpick == 236) return RadioInterceptor_Dialog_Main(playerid);
		else if(fpick == 237) return PDatabase_PlayerTakeThermite(playerid);
		else if(fpick == 238) return pc_cmd_thing(playerid), i_resetveshi(playerid); // Алюминий
		else if(fpick == 249) return format(string, sizeof(string), "%d", inva), pc_cmd_usepads(playerid, string), i_resetveshi(playerid); // Прокладки
