...
// 5. TopThing - топовые вещи для кейса
...
new fdrawName[][] = // Название Вещи
{
	...
	"KOЏE‡OK", "KYЊOH OѓE„ѓA", "KYЊOH AKCECCYAP", "KYЊOH A‹ЏO", "KYЊOH KP…ЊЏA"
};
new fdrawNameEN[][] = // Название Вещи на Английском
{
	...
	"CAULDRON", "COUPON CLOTHES","COUPON ACCESSORY","COUPON CAR","COUPON CRYPTA"
};
...
new friskName[][] = // Название Вещи
{
	...
	"Детектор радаров", "Улучшенный детектор радаров", "Глушилка радаров", "Ключ от Кейса Маньяка", "Ключ от Кейса Yakuza", "Радиоперехватчик", // 231 - 236
	"Термитная смесь", "Алюминий", "Пропуск Годжо", "Киловатт", "Ключ от Похоронного Кейса", "Череп Осириса", "Некрономикон", "Мех Зайца", "Мех Лисы", "Шкура Волка", "Шкура Медведя", "Кожа Оленя","Прокладки","Маленький Котелок", // 237-250
	"Большой Котелок", "Купон одежда", "Купон аксессуар","Купон транспорт","Купон крипта" // 251 - 255
};
...
new friskPick[] = // ID Модельки в Инвентаре (обычный предмет)
{
	...
	12375, 12446, 12447, 12445, 12451 // 251 - 255
};
...
stock CheckThingQuan(t) // Имеет ли предмет количество (1 да, 0 нет)
{
	...
	|| t == 181 || t == 183 || t >= 190 && t <= 192 || t == 197 || t == 225 || t == 230 || t == 238 || t >= 252 && t <= 255) ? 1 : 0;
}
...
stock get_invent2_Slot(playerid, thingId, thingType) // Поиск предмета в инвентаре (Только две страницы основного инвентаря) и возвращаем slot
{
	new slot = -1;
	for(new i = 0; i < 40; i++)
	{
		if(PlayerInfo[playerid][pInven][i] == thingId && PlayerInfo[playerid][pInvenType][i] == thingType && PlayerInfo[playerid][pInvenQuan][i] > 0) 
		{
			slot = i;
			break;
		}
...
stock get_invent3_Slot(playerid, thingId, thingType) // Поиск предмета в инвентаре (Только две страницы основного инвентаря) и возвращаем slot
{
	new slot = -1;
	for(new i = 0; i < 40; i++)
	{
		if(PlayerInfo[playerid][pInven][i] == thingId && PlayerInfo[playerid][pInvenType][i] == thingType) 
		{
			slot = i;
			break;
		}
...
stock PerishableThing(i, type) //  Проверка на портящиеся продукты
{
	...
	|| i == 168 || i == 169 || i == 170 || i == 171 || i == 172 || i == 173 || i == 174 || i == 233 || i == 236)) return 1;
	return 0;
}
...
{
	if (IsACasePackID(thingPack)) return 0; // Кейс можно выбрасывать и передавать всегда
...
{
	if(IsACasePackID(thingPack) // Предмет упакован в кейс
...
stock UseItem(playerid,inva, fpick,fquan,fpara,thingType,thingPack)
{
	...
	if(thingType == 0) // Обычный предмет
	{
		...
		*/
		...
		else if(fpick >= 252 && fpick <= 255) return format(string, sizeof(string), "%d", inva), UseCoupon(playerid, string), i_resetveshi(playerid); // Котелок
