...
#define INVENTER 190 // Максимальный ID (обычный предмет)
...
new fdrawName[][] = // Название Вещи
{
	...
	"3AЊЌACЏ’ ЂOMЂ‘","PEMOHTHSN_HA6OP","KPACKA_DLR_ABTOMO6NLR","ABTOMO6NLJHSE_DNCKN","FNDPABLNKA","3AKNCJ_A3OTA","DETALJ_TUHNHFA","KENC"
};
new fdrawNameEN[][] = // Название Вещи на Английском
{
	...
	"BOMB SPARE PART","REPAIR KIT","CAR PAINT","CAR WHEELS","HYDRAULICS","NITROUS OXIDE","TUNING COMPONENT","CASE"
};
new friskName[][] = // Название Вещи
{
	...
	"Запчасть бомбы","Ремонтный Набор","Краска для автомобиля","Автомобильные диски","Гидравлика","Закись Азота","Деталь Тюнинга","Кейс" // 182 - 189
};
new friskPick[] = // ID Модельки в Инвентаре (обычный предмет)
{
	...
	3082,0,0,0,0,0,0,19918 // 182 - 189
};
...
new friskDefault[] = // Гос. стоимости предметов
{
	...
	1000,1500,3500,1500,7500,10000,10000,0 // 189
};
	...
	else if(fpick > 0) // Что-то Лежит
	{
        ...
        else if(OnlineInfo[playerid][oInventSelectLeft] != 9999 && OnlineInfo[playerid][oInventSelectLeft] == inva) // Выполняем
		{
		    ...
		    if(thingType == 0) // Обычный предмет
		    {
				...
				{
					ShowDialog(playerid,1335,DIALOG_STYLE_INPUT,"{ff9000}Бомба","\n{cccccc}Введите время, через которое бомба взорвётся [ 5 - 10 минут ]","Принять","Отмена");
					return 1;
				}
