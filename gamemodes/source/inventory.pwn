...
new friskName[][] = // Название Вещи
{
	...
	"Запчасть бомбы","Рем.комплект Авто","Краска для транспорта","Автомобильные диски","Гидравлика","Закись Азота","Деталь Тюнинга","Кейс","Рем.комплект Мото","Рем.комплект Авиа", // 182 - 191
		...
		if(item >= 1)
		{
			...
			else if(thingPack == 0) // Без упаковки
			{
				if(thingType == 0) // Обычный предмет
				{
					...
					if(CheckThingQuan(item) == 1) // Количественный
					{
						new string[8];
        ...
        else if(OnlineInfo[playerid][oInventSelectLeft] != 9999 && OnlineInfo[playerid][oInventSelectLeft] == inva) // Выполняем
		{
		    ...
		    if(thingType == 0) // Обычный предмет
		    {
			 	...
			 	else if(fpick == 11)
				{
		 		...
		 		else if(fpick == 60) return cmd_palladium(playerid), i_resetveshi(playerid); // Палладий
