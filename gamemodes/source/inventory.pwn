...
new fdrawName[][] = // Название Вещи
{
	...
	"CK…PH","KO‡ЂACA","KOHCEP‹‘ ѓ‡• KOЏO‹","KOHCEP‹‘ ѓ‡• COЂAK","‡AKOMCЏ‹O ѓ‡• Њ…ЏOM‰E‹","‡EKAPCЏ‹O ѓ‡• Њ…ЏOM‰E‹"
};
new fdrawNameEN[][] = // Название Вещи на Английском
{
	...
	"SKIRN","SAUSAGE","CANNED FOOD FOR CATS","CANNED FOOD FOR DOGS", "TREATS FOR PETS","MEDICINE FOR PETS"
};
...
new friskName[][] = // Название Вещи
{
	...
	"Скирк","Колбаса","Консервы для кошек","Консервы для собак","Лакомство для питомцев","Лекарство для питомцев" // 261 - 266
};
...
stock TopThing(i)
{
	...
	|| i == 222 || i == 223 || i == 224 || i == 239 || i == 241 || i == 242) return true;
	return false;
}

new friskPick[] = // ID Модельки в Инвентаре (обычный предмет)
{
    0,19579,2710,1520,19473,12345,1578,1279,11738,1650,19515, // 0 - 10
	...
	19569,12727,12728,12729,12730,12731 // 261-266
};
...
stock CheckThingQuan(t) // Имеет ли предмет количество (1 да, 0 нет)
{
	...
	|| t == 181 || t == 183 || t >= 190 && t <= 192 || t == 197 || t == 225 || t == 230 || t == 238 || t >= 252 && t <= 260) ? 1 : 0;
}
...
new friskDefault[] = // Гос. стоимости предметов
{
	...
	-1, -1, 10000,10000,500000,50000 // 266
};
...
stock ThingParameters(playerid, thingId, &quan, &para)
{
	...
	else if(thingId == 261 && para == 0) SetSatiety(thingId, quan), para = unix + 172800; // Скирн
...
stock i_limit(playerid, thingId, &getQuan, &getLimit) // Проверяем лимиты инвентаря
{
	...
	lim[4] = 100*pow, lim[5] = 100*pow, lim[6] = 100*pow, lim[7] = 100*pow, lim[9] = 20, lim[18] = 100*pow, lim[20] = 20*pow,lim[22] = 10*pow;
