...
CMD:gthing(playerid, const params[]) // Выдать предмет (НЕ ИСПОЛЬЗОВАТЬ эту команду без надобности, она не учитывает кучу условий для разных предметов)
{
  new itemname[64];
  if(PlayerInfo[playerid][pSoska] < 20) return ErrorMessage(playerid, "{FF6347}Только для Системных администраторов");
  if(sscanf(params, "is[64]i",params[0],itemname,params[1])) return SendClientMessage(playerid, COLOR_GREY, "[ Мысли ]: Выдать предмет в инвентарь [ ID, Предмет, ID/Name, Кол-во ]");
...
stock UnlegalThing(i, thingType, thingPack)
{
	if(thingType == 0 // Обычный предмет
		...
		&& (i == 4 || i == 5 || i == 6 || i == 7 || i == 10 || i == 11 || i == 12 || i == 140 || i == 19 || i == 90 || i == 182 || i == 205)) return 1;
	return 0;
}
