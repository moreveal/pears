...
stock goloadInterior(playerid, userId, str_name[]) // Начинаем загрузку интерьера
{
    ...
    format(string_mysql,sizeof(string_mysql),"SELECT * FROM `pp_peo_information` WHERE `userId` = '%d'", userId);
	mysql_tquery(pearsq, string_mysql, "Call_loadinterior_information", "ddds", playerid, g_MysqlRaceCheck[playerid], userId, str_name);
    return 1;
}
...
stock sqlite_LoadMapObjects(playerid)
{
	...
	stmt_close(loadstmt);
    format(store,sizeof(store),"{444444}Маппинг с интерьером загружен {A86CFB}[Объектов: %d]", count);
    SuccessMessage(playerid, store);
    return 0;
}