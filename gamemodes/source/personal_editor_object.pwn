...
stock useAvailableInterior(playerid, propId, typeProperty)
{
    ...
    if(IsPlayerEditObject(playerid)) return ErrorMessage(playerid, "{FF6347}Завершите редактирование объекта");
...
{
    if(!peoInfo[playerid][peoInEditor]) return ErrorMessage(playerid, "{FF6347}Вы не находитесь в редакторе [ /loadeditor ]");
    if(IsPlayerEditObject(playerid)) return ErrorMessage(playerid, "{FF6347}Вы уже используете редактор объектов");
...
stock CreateObjectPersonalEditor(playerid, modelId, world, interior) // Создание объекта в личном редакторе
{
    ...
    GoEditDynamicObject(playerid, REDAKT_TYPE_OWN_SET, 0, peoId, slotId, peoInfo[peoId][peoObject][slotId], 0);
    return 1;
}
...
stock EditObjectPersonalEditor(playerid, slotId, world) // Перемещение объекта в личном редакторе
{
    ...
    GoEditDynamicObject(playerid, REDAKT_TYPE_OWN_MOVE, 1, peoId, slotId, peoInfo[peoId][peoObject][slotId], 0);
    return 1;
}
...
CMD:loadinterior(playerid, const params[]) // Загружаем интерьер
{
    ...
    // Если не админ и ничего не вводили, то просто грузим собственный интерьер
