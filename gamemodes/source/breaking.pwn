...
stock CreateBreaking(playerid, type, breakingId, hardLevel) // Открываем мини игру для взлома
{
	...
	if(hardLevel == 1) return ErrorMessage(playerid, "{FF6347}Установлена сигнализация\n{ffcc66}Вам требуется навык взломщика 3+ [ /skill ]\nВы можете прокачивать навык на простых замках или приобрести в Y >> Donate");
	else if(hardLevel == 2) return ErrorMessage(playerid, "{FF6347}Установлена усложнённая сигнализация\n{ffcc66}Вам требуется навык взломщика 6+ [ /skill ]\nВы можете прокачивать навык на простых замках или приобрести в Y >> Donate");
	else if(hardLevel == 3) return ErrorMessage(playerid, "{FF6347}Установлена максимальная сигнализация\n{ffcc66}Вам требуется навык взломщика 9+ [ /skill ]\nВы можете прокачивать навык на простых замках или приобрести в Y >> Donate");
