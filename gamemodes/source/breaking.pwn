...
stock CreateBreaking(playerid, type, breakingId, hardLevel) // Открываем мини игру для взлома
{
	...
	{
		new string[200];
		format(string,sizeof(string), "{FF6347}Установлена сигнализация\n{ffcc66}Вам требуется навык взломщика %d+ [ /skill ]\
									\nВы можете прокачивать навык на простых замках или приобрести в Y >> Donate", needSkill);
