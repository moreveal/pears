...
/*
ДОДЕЛАТЬ
- Сделать ещё несколько маршрутов
*/
...
stock Dominic_StartQuest(playerid)
{
    ...
    {
        if(serverType > 0) return ErrorMessage(playerid, "{FF6347}Elon Musk: я не доделал систему"); // VREMENNO
        ...
        else
        {
            ...
            SetPlayerChatBubble(NPCInfo[5][npcID],"Уверен, что сможешь победить меня, малец?",0x67b2ffFF,5.0,3000);
...
stock Dominic_GiveGift(playerid, Float:x, Float:y, Float:z)
{
    ...
    SetPlayerChatBubble(NPCInfo[5][npcID],"Возьми.. Это для тебя",0x67b2ffFF,5.0,3000);
    return 1;
}
...
stock Dominic_PickupRaceCheckpoint()
{
    ...
    if(IsOnline(DominicPlayeridRace))
    {
        ...
        {
            PlayAudioStreamForPlayer(DominicPlayeridRace, "https://cdn.pears.fun/sound/characters/dominic/dominic5.mp3");
        ...
        {
            PlayAudioStreamForPlayer(DominicPlayeridRace, "https://cdn.pears.fun/sound/characters/dominic/dominic5_w.mp3");
...
{
    PlayAudioStreamForPlayer(playerid, "https://cdn.pears.fun/sound/characters/dominic/dominic1.mp3");
...
CMD:reloaddominic(playerid)
{
    ...
    ShowDialog(DominicPlayeridRace,1700,DIALOG_STYLE_MSGBOX,"{ff9000}Доминик Торпеда","{ffcc66}Ограничение, на повторную гонку с Домиником, очищено","*","");
    return 1;
}
...
stock Dominic_CheckDamageVehicle(playerid, vehicleid)
{
    if(VehInfo[vehicleid][vHealth] <= 1500.0 && DominicDamageReplic == 0)
    {
        ...
        PlayAudioStreamForPlayer(playerid, "https://cdn.pears.fun/sound/characters/dominic/dominic2.mp3");
        SendClientMessage(playerid, COLOR_YELLOW,"Доминик (голосовое): Ты как там? В порядке?");
