...
stock UpdateDrawInvaThing(playerid, slot) // Обновляем отображение ячеек для крафта
{
    ...
    if(InvaCraft[playerid][slot] == 0) // Пусто
    {
        ...
        PlayerTextDrawBackgroundColor(playerid, CraftProcessDraw[i][playerid], PlayerInfo[playerid][pStyle1]);
        PlayerTextDrawColor(playerid, CraftProcessDraw[i][playerid], PlayerInfo[playerid][pStyle1]);
        PlayerTextDrawBoxColor(playerid, CraftProcessDraw[i][playerid], 0);
    }
    else // Чё-то есть
    {
        PlayerTextDrawFont(playerid, CraftProcessDraw[i][playerid], 5);
        PlayerTextDrawBackgroundColor(playerid, CraftProcessDraw[i][playerid], PlayerInfo[playerid][pStyle1]);
        PlayerTextDrawColor(playerid, CraftProcessDraw[i][playerid], -1);
        PlayerTextDrawBoxColor(playerid, CraftProcessDraw[i][playerid], 0);
...
stock UpdateDrawCraftThing(playerid, thingId) // Обновляем отображение выбранного предмета
{
    ...
    if(thingId == 0)
    {
        ...
        PlayerTextDrawColor(playerid, CraftProcessDraw[1][playerid], 100); // -1
...
stock createDraw_CraftProcess(playerid, type)
{
	...
	for(new i = 17; i <= 21; i ++)
	{
		...
		PlayerTextDrawAlignment(playerid, CraftProcessDraw[i][playerid], 1);
		if(PlayerInfo[playerid][pSty] == 6 || PlayerInfo[i][pSty] == 11) PlayerTextDrawColor(playerid, CraftProcessDraw[i][playerid], 673720575);
		else PlayerTextDrawColor(playerid, CraftProcessDraw[i][playerid], -1);
		PlayerTextDrawSetShadow(playerid, CraftProcessDraw[i][playerid], 0);
		PlayerTextDrawSetOutline(playerid, CraftProcessDraw[i][playerid], 0);
		PlayerTextDrawBackgroundColor(playerid, CraftProcessDraw[i][playerid], 51);
