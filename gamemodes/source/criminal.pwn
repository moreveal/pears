    ...
    for(new i = minlist; i < MAX_CRIMINAL_CODE_ARTICLE; i++)
	{
        if(CriminalCodeInfo[i][0][ccStatus] == true) // Только если основная статья существует
        {
            ...
            for(new p = 0; p < MAX_CRIMINAL_CODE_SUBENTRY; p++)
	        {
                ...
                format(line,sizeof(line),"\n%s%s\t%s%s\t{FF6347}%s\t{FF6347}%s", ukshow(p), CriminalCodeInfo[i][p][ccArticle], ukshow(p), CriminalCodeInfo[i][p][ccName], textCrime, textFine);
                strcat(lines,line);
...
stock CriminalCodeSetting(playerid, i, p) // Меню редактора статьи
{
    ...
    {
        format(line,sizeof(line),"%s%s %s\t ", ukshow(p), CriminalCodeInfo[i][p][ccArticle], CriminalCodeInfo[i][p][ccName]), strcat(lines,line);
        format(line,sizeof(line),"\n{cccccc}Статус: \t{99ff66}[ On ]"), strcat(lines,line);
    }
	format(line,sizeof(line),"\n{cccccc}Номер: \t %s", CriminalCodeInfo[i][p][ccArticle]), strcat(lines,line);
...
stock ShowDialogCriminalCodeInfo(playerid, i, p)
{
    ...
    format(line,sizeof(line),"\n%s%s %s\n", ukshow(p), CriminalCodeInfo[i][p][ccArticle], CriminalCodeInfo[i][p][ccName]), strcat(lines,line);
...
stock CriminalCodeSave(i, p) // Сохраняем в базу (моментальное сохранение при любом изменении)
{
    ...
    {
        mysql_format(pearsq, string_mysql,sizeof(string_mysql),"INSERT INTO `criminal_code` (ccI, ccP, ccStatus, ccArticle, ccName, ccLevel, ccFine,\
            ccText, ccUnix, ccPlayer, ccUserID) VALUES ('%d', '%d', '%d', '%e', '%e', '%d', '%d', '%e', '%d', '%e', '%d')",
        i, p, CriminalCodeInfo[i][p][ccStatus], CriminalCodeInfo[i][p][ccArticle], CriminalCodeInfo[i][p][ccName], CriminalCodeInfo[i][p][ccLevel], CriminalCodeInfo[i][p][ccFine], 
        CriminalCodeInfo[i][p][ccText], CriminalCodeInfo[i][p][ccUnix], CriminalCodeInfo[i][p][ccPlayer], CriminalCodeInfo[i][p][ccUserID]);
        mysql_tquery(pearsq, string_mysql, "Call_InsertCriminal", "dd", i, p);
    }
    else
    {
        mysql_format(pearsq, string_mysql,sizeof(string_mysql),"UPDATE `criminal_code` SET `ccStatus` = '%d', `ccArticle` = '%e', `ccName` = '%e', `ccLevel` = '%d', `ccFine` = '%d',\
            `ccText` = '%e', `ccUnix` = '%d', `ccPlayer` = '%e', `ccUserID` = '%d' WHERE `newid` = '%d'",
        CriminalCodeInfo[i][p][ccStatus], CriminalCodeInfo[i][p][ccArticle], CriminalCodeInfo[i][p][ccName], CriminalCodeInfo[i][p][ccLevel], CriminalCodeInfo[i][p][ccFine], 
        CriminalCodeInfo[i][p][ccText], CriminalCodeInfo[i][p][ccUnix], CriminalCodeInfo[i][p][ccPlayer], CriminalCodeInfo[i][p][ccUserID], 
...
stock PutSubentryLoad(f, i, p, bool:status)
{
    ...
    cache_get_value_name(f, "ccArticle", CriminalCodeInfo[i][p][ccArticle], 8);
...
{
	if(IsPoliceMember(playerid) || PlayerInfo[playerid][pFbi] >= 1 || PlayerInfo[playerid][pSoska] >= 1)
	{
...
stock ShowPlayerWanted(playerid, criminalid)
{
    ...
    for(new i = 0; i < MAX_CRIME_PLAYER; i++)
    {
        ...
        format(line,sizeof(line),"\n{ff9000}%s\t{FF6347}%s\t{0066ff}%s\t{555555}%02d.%02d.%d %02d:%02d",CriminalCodeInfo[uk][p][ccArticle],
...
stock ShowPlayerSettingTicket(playerid, i)
{
    ...
    format(line,sizeof(line),"%s\t{FF6347}%s\t{0066ff}%s",CriminalCodeInfo[uk][p][ccArticle],CriminalCodeInfo[uk][p][ccName],TicketPolice[criminalid][i]), strcat(lines,line);
...
stock ClearPlayerTicketArcticle(playerid, criminalid)
{
    ...
    new string[80];
    format(string,sizeof(string),"%s %s", CriminalCodeInfo[uk][p][ccArticle],CriminalCodeInfo[uk][p][ccName]);
...
stock ShowPlayerSettingWanted(playerid, i)
{
    ...
    new line[150],lines[300];
    format(line,sizeof(line),"%s\t{FF6347}%s\t{0066ff}%s",CriminalCodeInfo[uk][p][ccArticle],CriminalCodeInfo[uk][p][ccName],WantedPolice[criminalid][i]), strcat(lines,line);
...
stock SaveTicketPlayer(playerid, i)
{
    ...
    mysql_format(pearsq, string_mysql,sizeof(string_mysql),"UPDATE `pp_wanted` SET `wanTicketCrime%d`='%d', `wanTicketSubentry%d`='%d', \
        `wanTicketPoliceId%d`='%d', `wanTicketUnix%d`='%d', `WantedTicketPolice%d`='%e' WHERE `playerid`='%d'", 
    ...
    mysql_format(pearsq, string_mysql,sizeof(string_mysql),"UPDATE `pp_igroki` SET `Ammo11` = '%d' WHERE `user_id` = '%d'", 
        PlayerInfo[playerid][pAmmos11], PlayerInfo[playerid][pID]);
	query_empty(pearsq, string_mysql);
    return 1;
}
...
{
    new string_mysql[400];
    mysql_format(pearsq, string_mysql,sizeof(string_mysql),"UPDATE `pp_wanted` SET `wanCrime%d`='%d', `wanSubentry%d`='%d', `wanPoliceId%d`='%d', \
        `wanUnix%d`='%d', `WantedPolice%d`='%e' WHERE `playerid`='%d'", 
    ...
    mysql_format(pearsq, string_mysql,sizeof(string_mysql),"UPDATE `pp_igroki` SET `Crimes` = '%d' WHERE `user_id` = '%d'", 
        PlayerInfo[playerid][pCrimes], PlayerInfo[playerid][pID]);
	query_empty(pearsq, string_mysql);
    return 1;
}
...
stock FindCriminalArticle(const article[], &uk, &p, bool: ignorecase = true) {
    for(new i = 0; i < MAX_CRIMINAL_CODE_ARTICLE; i++)
    {
        if(CriminalCodeInfo[i][0][ccStatus] == false) continue;
        for(new s = 0; s < MAX_CRIMINAL_CODE_SUBENTRY; s++)
        {
            if(strfind(CriminalCodeInfo[i][s][ccArticle], article, ignorecase) != (-1) || strfind(CriminalCodeInfo[i][s][ccName], article, ignorecase) != (-1))
            {
                uk = i;
                p = s;
                return 1;
...
CMD:su(playerid, const params[])
{
    ...
    if(!sscanf(params, "s[121]s[64]", tmp, article)) // Указываем Ник и Номер статьи
    {
        ...
        new findUk, findP;
        if (!FindCriminalArticle(article, findUk, findP)) return ErrorMessage(playerid, "{FF6347}Номер статьи не найден");
...
stock SetPlayerCriminal(playerid, zakonnik, const reason[], zv, uk, p)
{
    ...
    if (PlayerInfo[playerid][pSoska] > 0 && PlayerInfo[playerid][pSoska] < 22)
    {
        ...
        if (IsOnline(zakonnik)) ErrorMessage(zakonnik, "{FF6347}Нельзя выдать розыск администратору");
        return -1;
    }
