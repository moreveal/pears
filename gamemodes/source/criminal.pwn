...
stock SetPlayerCriminal(stat,playerid,zakonnik,const reason[],zv, uk) // 0 - розыск дала система 1 - розыск дал человек, ID Преступника, ID Мента, Причина, Количество Звёзд
{
	...
	if(OnlineInfo[playerid][oLogged] == 1)
	{
        ...
        if(slotUk >= 0 && CriminalCodeInfo[uk][ccLevel] != 0)
		{
			...
			if(stat == 1) format(string, sizeof(string), "{abcdef}Вы совершили Преступление [%s] Полицейский: [%s] {FF6347}Ур. розыска: [%d]",reason,PlayerInfo[zakonnik][pName],zv);
			...
			foreach (Player, i)
			{
				...
				{
					if(stat == 1) format(string, sizeof(string), "[%s]: Преступление: [%s], Подозреваемый: [%s], Ур. розыска: [%d]",PlayerInfo[zakonnik][pName],reason,rpplayername(playerid),zv);
