...
stock CriminalCodeSave(i, p) // Сохраняем в базу (моментальное сохранение при любом изменении)
{
    ...
    {
        mysql_format(pearsq, string_mysql,sizeof(string_mysql),"INSERT INTO `criminal_code` (ccI, ccP, ccStatus, ccArcticle, ccName, ccLevel, ccFine,\
            ccText, ccUnix, ccPlayer, ccUserID) VALUES ('%d', '%d', '%d', '%e', '%e', '%d', '%d', '%e', '%d', '%e', '%d')",
        i, p, CriminalCodeInfo[i][p][ccStatus], CriminalCodeInfo[i][p][ccArcticle], CriminalCodeInfo[i][p][ccName], CriminalCodeInfo[i][p][ccLevel], CriminalCodeInfo[i][p][ccFine], 
    ...
    {
        mysql_format(pearsq, string_mysql,sizeof(string_mysql),"UPDATE `criminal_code` SET `ccStatus` = '%d', `ccArcticle` = '%e', `ccName` = '%e', `ccLevel` = '%d', `ccFine` = '%d',\
            `ccText` = '%e', `ccUnix` = '%d', `ccPlayer` = '%e', `ccUserID` = '%d' WHERE `newid` = '%d'",
        CriminalCodeInfo[i][p][ccStatus], CriminalCodeInfo[i][p][ccArcticle], CriminalCodeInfo[i][p][ccName], CriminalCodeInfo[i][p][ccLevel], CriminalCodeInfo[i][p][ccFine], 
        CriminalCodeInfo[i][p][ccText], CriminalCodeInfo[i][p][ccUnix], CriminalCodeInfo[i][p][ccPlayer], CriminalCodeInfo[i][p][ccUserID], 
