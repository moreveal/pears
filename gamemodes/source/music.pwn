...
stock CreateVehicleMusicStream(vehicleid, const url[])
{
    if (IsVehicleHavingMusicStream(vehicleid)) {
        return SetVehicleMusicStreamUrl(vehicleid, url);
    }
    ...
    SetVehicleMusicStreamUrl(vehicleid, url);
    return true;
}

stock SetVehicleMusicStreamUrl(vehicleid, const url[])
{
    if (!IsVehicleHavingMusicStream(vehicleid)) return false;

    SetAudioStreamUrl(VehicleMusicAudioStream[vehicleid], url);
    return true;
}
