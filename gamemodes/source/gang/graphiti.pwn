...
stock GraphitiUpdateElement(graphiti)
{
    ...
    // Натягиваем текстуру на граффити
    ...
    new string[100];
    format(string,sizeof(string),"{cccccc}Граффити %s\n\n{666666}[ Баллончик или Канистра в руках + ALT ]", frakeasyName[g]);
...
stock SaveGraphiti(slot)
{
    ...
    mysql_format(pearsq, string_mysql, sizeof(string_mysql), "UPDATE `pp_graphiti` SET `gstring`='%e',`gunix`='%d',`gplayernumber`='%d',`gZone`='%d',`gStatus`='%d',`gorg`='%d',`gName`='%e' WHERE `gnewid`='%d'",
