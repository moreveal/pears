...
stock dialogCase_Promo(playerid, dialogid, response, listitem,const inputtext[])
{
  	...
  	if(dialogid == 622)
	{
  	    if(response)
  	    {
			...
			if(listitem == 8) ShowDialog(playerid,626,DIALOG_STYLE_INPUT,"{ff9000}Промокод","{cccccc}Введите уровень с которого игрок сможет взять промокод (0 при регистрации) [0 - 100]","Принять","Отмена");
			if(listitem == 9) ShowDialog(playerid,603,DIALOG_STYLE_INPUT,"{ff9000}Промокод","{cccccc}Введите доступное количество активаций этого промокода (0 неограничено) [0 - 100.000]","Принять","Отмена");
			...
			if(listitem == 11) ShowDialog(playerid,629,DIALOG_STYLE_INPUT,"{ff9000}Промокод","{cccccc}Введите количество дней через которое промокод {99ff66}начнёт своё действие [0 - 3000]","Принять","Отмена");
			if(listitem == 12) ShowDialog(playerid,614,DIALOG_STYLE_INPUT,"{ff9000}Промокод","{cccccc}Введите количество дней через которое промокод {FF6347}завершит своё действие [1 - 3000]","Принять","Отмена");
			if(listitem == 13) ShowDialog(playerid,606,DIALOG_STYLE_MSGBOX,"{ff9000}Промокод","{cccccc}Вы уверены, что хотите удалить промокод?\n\n","Да","Отмена");
  	    }
 	...
 	if(dialogid == 624)
	{
  	    if(response)
  	    {
  	        new pf = ListCode[playerid];
  	        if(PromoInfo[pf][roActiv] == 1) return ErrorText(playerid,"{FF6347}Нельзя редактировать активный промокод"), setprom(playerid);
  	    	if(listitem == 0) ShowDialog(playerid,611,DIALOG_STYLE_INPUT,"{ff9000}Промокод","{cccccc}Введите количество денег {99ff66}1.000$ - 300.000$","Принять","Отмена");
  	    	...
  	    	if(listitem == 2) ShowDialog(playerid,609,DIALOG_STYLE_INPUT,"{ff9000}Промокод","{cccccc}Введите Level, на который повысится игрок {ff9000}1","Принять","Отмена");
