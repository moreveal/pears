...
stock AddCustomSkins()
{
	...
	AddCharSyncModel(42, 414); // 15601, prisonmex male
	AddCharSyncModel(304, 415); // 15602, pearscop male
    return 1;
}
...
stock skinprice(playerid, page) // Настройки гос. цен одежды
{
    ...
    if((minlist >= 311 + MAX_SKIN_CUSTOM || DP[5][playerid] == 1) && thisPage == 0) // Сбрасываем страницы, если последний лист максимальный или больше
	{
		DP[1][playerid] = 0; // Страница
		DP[2][playerid] = 0; // Последний list на странице
		DP[4][playerid] = 0; // Первый list на странице
...
stock SkinPriceSorting(playerid)
{
    ...
    ShowDialog(playerid,982,DIALOG_STYLE_TABLIST_HEADERS,"Гос Стоимость Одежды",lines,"Выбрать","Назад");
    return 1;
}
...
{
	if(dialogid == 982) // Настройки фильтра
	{
        if(response)
        {
            if(listitem == 0)
            {
				ShowDialog(playerid,980,DIALOG_STYLE_INPUT,"Фильтр","{cccccc}Введите ID одежды","Принять","Отмена");
            }
            if(listitem == 1)
            {
                ShowDialog(playerid,972,DIALOG_STYLE_INPUT,"Фильтр","{cccccc}Введите название одежды \nМожно не полное название\n1 - 30 символов","Принять","Отмена");
            }
            if(listitem == 2) // Сбросить Фильтр
            {
				ReloadSorting(playerid, 1075);
				SkinPriceSorting(playerid);
            }
        }
        else skinprice(playerid, 0);
	}
	else if(dialogid == 980) // Фильтр по id
	{
        if(response)
        {
			...
			if(input < 1 || input > 1000) return ErrorText(playerid, "{FF6347}Не меньше 1 и не больше 1000 (1 - 311, 312 и выше - кастомные)"), SkinPriceSorting(playerid);
			OnlineInfo[playerid][oSorting][1] = input;
            PlayerPlaySound(playerid,6401,0,0,0);
            SkinPriceSorting(playerid);
        }
        else SkinPriceSorting(playerid);
    }
	else if(dialogid == 972) // Фильтр по названию
	{
        if(response)
        {
			if(!strlen(inputtext)) return ErrorText(playerid, "{FF6347}Вы ничего не ввели"), SkinPriceSorting(playerid);
			if(strlen(inputtext) < 1 || strlen(inputtext) > 30) return ErrorText(playerid, "{FF6347}1 - 30 символов"), SkinPriceSorting(playerid);
           	if(checksimvol(inputtext)) return ErrorText(playerid, "{FF6347}Вы используете запрещённый символ"), SkinPriceSorting(playerid);
	...
	else if(dialogid == 1075)
	{
		if(response)
		{
			if(listitem == 0) SkinPriceSorting(playerid);
			...
			if(DP[0][playerid] > 0) // Есть строки на странице
			{
				...
				else if(listitem == DP[0][playerid] + 1) skinprice(playerid, DP[1][playerid] + 1); // Следующая страница
			}
		}
	...
	else if(dialogid == 971)
	{
		if(response)
		{
			...
			else if(listitem == 2)
			{
				...
				skinsaleUpdate = true;
				skinprice(playerid, DP[1][playerid]);
			}
		}
		else skinprice(playerid, DP[1][playerid]);
	}
	else if(dialogid == 970)
	{
		if(response)
		{
			...
			SettingGosPriceSkin(playerid, DP[3][playerid]);
		...
		else SettingGosPriceSkin(playerid, DP[3][playerid]);
	}
