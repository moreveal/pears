...
new Float:SupermarketItemPos[][] =
{
	...
	{ 1124.570190, -1380.577636, 1401.935424 } // 23 мясо в упакове
};

#define MAX_BIZ_ITEMSUPERMARKET 24
/* 
	Как добавить новый товар в супермаркет?
	1. Добавить в biz/biz.pwn stock LoadBusinessProduct - следующий предмет
	2. Увеличить цифру в MAX_BIZ_ITEMSUPERMARKET
	3. Добавить координату предмета, который лежит где-то на полке в супермаркете: Float:SupermarketItemPos
	4. По необходимости дополнить маппинг супермаркета новыми предметами
*/
...
CMD:supermarket(playerid) // Взять тележку
{
	if(IsPlayerInRangeOfPoint(playerid,2.0,1113.8209,-1384.0480,1401.7142) && GetPlayerInterior(playerid) == 206 && GetPlayerVirtualWorld(playerid) >= 1)
	{
		...
		if(PPP15[playerid] != 4)
		{
			if(OnlineInfo[playerid][oInHandThing][0] > 0 || Hand[playerid] >= 1 || GetPlayerWeapon(playerid) >= 2 || Hold[playerid] >= 1) return ErrorMessage(playerid, "{FF6347}У вас заняты руки [ Предмет или оружие ]");
			new b = GetPlayerVirtualWorld(playerid)+12;
			SetPVarInt(playerid,"sbussines",b);
			SetPVarInt(playerid,"svzyal", 1);
			PPP15[playerid] = 4;
			ApplyAnimation(playerid,"MISC","bmx_idleloop_02",4.1,1,1,1,1,1);
