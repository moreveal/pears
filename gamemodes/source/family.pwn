...
#define MAX_NAME_FAMILY_LENGTH 65 // Длинна названий в семьях (Если указано 65, значит максимальное количество символов 64, всегда -1 слот)
...
#define MAX_VEH_FAMILY 10 // Максимальное количество авто в гараже семьи
...
CMD:famrefundcar(playerid) {
	if (PlayerInfo[playerid][pSoska] < 22) return SendClientMessage(playerid, COLOR_GREY, "[ Мысли ]: Я не могу это сделать..");

	new countFam[MAX_FAMILY], countCar[MAX_FAMILY];
	for (new f = 0; f < MAX_FAMILY; f++) {
		if (!FamilyInfo[f][fStatusGarage]) continue;
		
		for(new i = 0; i < MAX_VEH_FAMILY; i++)
		{
			new model = FamilyInfo[f][fVeh][i];
			if (model == 0) continue;
			
			if (GetVehiclePrice(model) == 0 || !GetVehicleSale(model)) {
				new price = GetVehiclePriceGos(model);
				FamilyInfo[f][fVeh][i] = 0;
				FamilyInfo[f][fMoney] += price;

				for (new j = 0; j < sizeof(countFam); j++) {
					if (countFam[j] == 0 || countFam[j] == f) {
						countFam[j] = f;
						countCar[j]++;
						break;
					}
				...
				new log_str[200];
				format(log_str, sizeof(log_str), "Возврат денег за недоступный для покупки транспорт [Model: %s / ID: %d]", GetVehicleName(model), model);
				FamilyLog(f, "famrefundcar", 0, "", "", 0, "", "", price, log_str);
			}
		}
	}

	new dialog_text[4096];
	for (new i = 0; i < sizeof(countFam); i++) {
		if (countFam[i] == 0) {
			if (i == 0) {
				SendClientMessage(playerid, COLOR_GREY, "[ Мысли ]: Не найдено ни одной семьи с недоступным транспортом");
				return 1;
			}
			break;
		}
		...
		format(dialog_text, sizeof(dialog_text),
			"%s{ff9000}Семья №%d. {cccccc}Удалено: %d шт.\n",

			dialog_text,
			countFam[i], countCar[i]
		);
	}

	return ShowDialog(playerid, 1700, DIALOG_STYLE_MSGBOX, "{ff9000}Недоступный транспорт семей", dialog_text, " * ", "");
}
...
stock AddFamilyVehicle(playerid, f, type)
{
	...
	if(VehInfo[vehicleid][vIdvlad] != playerid || PlayerInfo[playerid][pKeyVehID] == vehicleid) return ErrorMessage(playerid, "{FF6347}Вы не можете установить в гараж чужой транспорт");
	...
	new playerslot = VehInfo[vehicleid][vDatabase];
	new vehsum = 0;

	for (new i = 0; i < MAX_VEH_FAMILY; i++) {
		if (FamilyInfo[f][fVeh][i] == model) {
			return ErrorMessage(playerid, "{FF6347}Этот транспорт уже есть в семейном гараже");
		}
	}

	if (server != 0) {
		if(Cars[vehicleid] == 9999 && PlayerInfo[playerid][pSoska] < 20) return ErrorMessage(playerid, "{FF6347}Вы не можете установить этот транспорт [ Создан Администрацией ]");
		if(GetVehiclePrice(model) == 0 || !GetVehicleSale(model)) return ErrorMessage(playerid, "{FF6347}Этот транспорт нельзя установить в гараж семьи");
	}
	
	if(type == 0)
	{
		vehsum = GetVehiclePriceGos(model) / 2;
		if(oGetPlayerMoney(playerid) < vehsum) return format(string,sizeof(string),"[ Мысли ]: Мне не хватает денег {ff0000}[ %d$ ]",vehsum), SendClientMessage(playerid, COLOR_GREY, string);
		GameTextForPlayer(playerid, "~n~~n~~n~~n~~n~~w~Џpa®cЈop¦ ~g~Yc¦a®oўћe®", 5000, 3);
		PlayerPlaySound(playerid, 6401, 0.0, 0.0, 0.0);
		format(string, sizeof(string), "{66ffff}Family {ffcc00}%s установил в гараж семьи %s", PlayerInfo[playerid][pName],GetVehicleName(model));
		SendFamilyMessage(f, COLOR_YELLOW, string);
		format(string, sizeof(string), "Установил %s (Тип 1)", GetVehicleName(model));
		FamilyLog(f, "famcar", PlayerInfo[playerid][pID], PlayerInfo[playerid][pName], PlayerInfo[playerid][pPlaIP], 0, "", "", 0, string);
		oGivePlayerMoney(playerid, -vehsum);
		FamilyInfo[f][fVeh][freeSlot] = model;
		FamilyInfo[f][fUpdate] = 1;
		PP_SetVehicleToRespawn(vehicleid); // Тот транспорт удаляем
	...
	else
	{
		...
		format(string, sizeof(string), "{66ffff}Family {ffcc00}%s установил в гараж семьи %s", PlayerInfo[playerid][pName],GetVehicleName(model));
		SendFamilyMessage(f, COLOR_YELLOW, string);
		format(string, sizeof(string), "Установил %s (Тип 2)", GetVehicleName(model));
		FamilyLog(f, "famcar", PlayerInfo[playerid][pID], PlayerInfo[playerid][pName], PlayerInfo[playerid][pPlaIP], 0, "", "", 0, string);
		FamilyInfo[f][fVeh][freeSlot] = model;
		FamilyInfo[f][fUpdate] = 1;
		PlayerInfo[playerid][pMyVeh][playerslot - 1] = 0;
		PlayerInfo[playerid][pMyVehID][playerslot - 1] = 0;
		ACDestroyVehicle(vehicleid);

		mysql_tquery(pearsq, "START TRANSACTION;");

		new string_mysql[120];
		mysql_format(pearsq, string_mysql,sizeof(string_mysql),"DELETE FROM `pp_cars` WHERE `sost` = '%d' AND `slot` = '%d'", PlayerInfo[playerid][pID], playerslot);
        query_empty(pearsq, string_mysql);

		// Сохраняем авто
  		mysql_format(pearsq, string_mysql,sizeof(string_mysql),"UPDATE `pp_igroki` SET `MyVeh%d` = '0' WHERE `user_id` = '%d'", playerslot - 1, PlayerInfo[playerid][pID]);
        query_empty(pearsq, string_mysql);
		...
		format(string, sizeof(string), "Гараж семьи № %d", f);
		CarLog("famcar", PlayerInfo[playerid][pID], PlayerInfo[playerid][pName], PlayerInfo[playerid][pPlaIP], model, 0, string);
	}
	return 1;
}

stock GetFreeFamVehSlot(f)
{
	new freeSlot = -1;
	for(new i = 0; i < MAX_VEH_FAMILY; i++)
	{
		if(FamilyInfo[f][fVeh][i] == 0)
		{
			freeSlot = i;
			break;
		}
...
stock CreateFamlLabel(playerid, f)
{
	...
	new string[100];
	format(string,sizeof(string),"[F] %s", FamilyInfo[f][fName]);
