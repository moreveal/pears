...
stock d_limit(d, thingId, &getQuan, &getLimit) // Проверяем лимиты дома
{
	...
	lim[140] = 10000, lim[141] = 10000, lim[142] = 1000;
...
stock SaveDomAll(idx) // Сохранение всего шкафа по цилку
{
	...
	i, DomInfo[idx][dInvent][i], i, DomInfo[idx][dInv][i], i, DomInfo[idx][dInvPara][i], i, DomInfo[idx][dInvQara][i], i, DomInfo[idx][dInvType][i], i, DomInfo[idx][dInvPack][i]);
    format(big_query,sizeof(big_query),"%s WHERE `Ids` = '%d'", big_query, idx);
	query_empty(pearsq, big_query);
	
	format(big_query,sizeof(big_query),"UPDATE `pp_dom` SET `Invent20` = '%d', `Inv20` = '%d', `InvPara20` = '%d', `InvQara20` = '%d', `InvType20` = '%d', `InvPack20` = '%d'",
	DomInfo[idx][dInvent][20], DomInfo[idx][dInv][20], DomInfo[idx][dInvPara][20], DomInfo[idx][dInvQara][20], DomInfo[idx][dInvType][20], DomInfo[idx][dInvPack][20]);
	for(new i = 21; i < 40; i++) format(big_query,sizeof(big_query),"%s, `Invent%d` = '%d', `Inv%d` = '%d', `InvPara%d` = '%d', `InvQara%d` = '%d', `InvType%d` = '%d', `InvPack%d` = '%d'", big_query,
	i, DomInfo[idx][dInvent][i], i, DomInfo[idx][dInv][i], i, DomInfo[idx][dInvPara][i], i, DomInfo[idx][dInvQara][i], i, DomInfo[idx][dInvType][i], i, DomInfo[idx][dInvPack][i]);
    format(big_query,sizeof(big_query),"%s WHERE `Ids` = '%d'", big_query, idx);
	query_empty(pearsq, big_query);
	
	format(big_query,sizeof(big_query),"UPDATE `pp_dom` SET `Invent40` = '%d', `Inv40` = '%d', `InvPara40` = '%d', `InvQara40` = '%d', `InvType40` = '%d', `InvPack40` = '%d'",
	DomInfo[idx][dInvent][40], DomInfo[idx][dInv][40], DomInfo[idx][dInvPara][40], DomInfo[idx][dInvQara][40], DomInfo[idx][dInvType][40], DomInfo[idx][dInvPack][40]);
	for(new i = 41; i < 60; i++) format(big_query,sizeof(big_query),"%s, `Invent%d` = '%d', `Inv%d` = '%d', `InvPara%d` = '%d', `InvQara%d` = '%d', `InvType%d` = '%d', `InvPack%d` = '%d'", big_query,
	i, DomInfo[idx][dInvent][i], i, DomInfo[idx][dInv][i], i, DomInfo[idx][dInvPara][i], i, DomInfo[idx][dInvQara][i], i, DomInfo[idx][dInvType][i], i, DomInfo[idx][dInvPack][i]);
    format(big_query,sizeof(big_query),"%s WHERE `Ids` = '%d'", big_query, idx);
	query_empty(pearsq, big_query);
	
	format(big_query,sizeof(big_query),"UPDATE `pp_dom` SET `Invent60` = '%d', `Inv60` = '%d', `InvPara60` = '%d', `InvQara60` = '%d', `InvType60` = '%d', `InvPack60` = '%d'",
	DomInfo[idx][dInvent][60], DomInfo[idx][dInv][60], DomInfo[idx][dInvPara][60], DomInfo[idx][dInvQara][60], DomInfo[idx][dInvType][60], DomInfo[idx][dInvPack][60]);
	for(new i = 61; i < 80; i++) format(big_query,sizeof(big_query),"%s, `Invent%d` = '%d', `Inv%d` = '%d', `InvPara%d` = '%d', `InvQara%d` = '%d', `InvType%d` = '%d', `InvPack%d` = '%d'", big_query,
	i, DomInfo[idx][dInvent][i], i, DomInfo[idx][dInv][i], i, DomInfo[idx][dInvPara][i], i, DomInfo[idx][dInvQara][i], i, DomInfo[idx][dInvType][i], i, DomInfo[idx][dInvPack][i]);
    format(big_query,sizeof(big_query),"%s WHERE `Ids` = '%d'", big_query, idx);
	query_empty(pearsq, big_query);
	return 1;
