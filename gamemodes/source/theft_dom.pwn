    ...
    if(GetPlayerVirtualWorld(playerid) >= 1001 && GetPlayerVirtualWorld(playerid) <= 3000)
    {
        if(Hold[playerid] == 1)
		{
            ...
            else // Нашли предмет
            {
                ...
                OnlineInfo[playerid][oOnBackThing][0] = DomInfo[dom][dOmodel][findObject];
...
stock FindTheftDomObject(playerid, dom)
{
    ...
    {
        if(DomInfo[dom][dObject][i] > 0 && DomInfo[dom][dOmodel][i] > 0)
        {
            if(DomInfo[dom][dObject][i])
            {
                ...
                {
                    if(FindTheftInIkea(DomInfo[dom][dOmodel][i]))
                    {
                        findObject = i;
                        break;
                    }
