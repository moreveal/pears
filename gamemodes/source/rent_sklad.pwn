...
stock shift_rent(playerid, wh, getinva, putinva) // Перемещение предметов внутри инвентаря арендованного склада организации (с одной ячейки на другую)
{
	if(OnlineInfo[playerid][oShowInterfaceRent] == wh)
	{
		...
		foreach(Player,i)
		{
		    ...
		    if(OnlineInfo[playerid][oShowInterfaceRent] != OnlineInfo[i][oShowInterfaceRent]) continue;
			quanPlayer ++;
		}
		if(quanPlayer >= 2)
		{
...
stock putrentwh(wh, pick, kol, thingType)
{
	...
	if(put_inva == -1 && stopFind == false)
	{
 	...
 	if(put_inva >= 0)
 	{
	 	...
	 	foreach(Player,i)
		{
			if(OnlineInfo[i][oLogged] == 1 && OnlineInfo[i][oShowInterface] == 1 && OnlineInfo[i][oShowInterfaceRent] == wh) tilesklad(i, WhInfo[wh][wInvent][put_inva], WhInfo[wh][wInv][put_inva], put_inva, thingType);
		}
	}
	return put_inva;
}
stock TakeRentwh(wh, stat, kolvo, thingType, dopinf)
{
	...
	foreach(Player, i)
	{
		if(OnlineInfo[i][oLogged] == 1 && OnlineInfo[i][oShowInterface] == 1 && OnlineInfo[i][oShowInterfaceRent] == wh) tilesklad(i, WhInfo[wh][wInvent][dopinf], WhInfo[wh][wInv][dopinf], dopinf, WhInfo[wh][wInvType][dopinf]);
	}
	return 1;
}
