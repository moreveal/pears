...
new accessRightName[][] = // Команды и настройки в организации
{
    ...
    "Исключение из ЧС", // 24
    "/frisk /take", // 25
	"/mafia(-farm)", // 26 Забить стрелу [Mafia]
    "/gov", // 27
    "/numbercar", // 28
    "/gac настройки склада", // 29
    "/camera", // 30 установить камеру слежения [FBI]
    "Жучок и прослушка /tracking", // 31
    "/su /pursuit", // 32
    "/lawyer", // 33
    "/zarest", // 34
    "Установка собственных радаров", // 35
    "Управление чужими радарами", // 36
    "/cuff /uncuff", // 37
    "/stun /unstun", // 38
    "/u /ub /d /db - общая рация", // 39
    "Отображение имён", // 40
    ...
    "/appoint", // 52 Назначить лидера [Goverment] // Можно заменить
    "/suspend", // 53 Снять лидера [Goverment] // Можно заменить
    ...
    "/arestbiz /unarestbiz", // 58 Арестовать бизнес, снять арест с биза [Goverment]
    ...
    "/nametag", // 70 Отключить / Включить никнейм [ICA] - ВЫКЛЮЧЕНА У ХИТОВ
    "/sign", // 71 Сменить имя [ICA]
    "Тюрьма", // 72 Система Тюрьмы [Police]
    "Военно Морской Флот", // 73 Военно Морской Флот [NGSA]
    "/boom", // 74 Атомка [NGSA]
    "Доставка мед. оборудования", // 75 Доставка Мед Оборудования [ASGH]
    "Тренировочные маршруты", // 76 /obstacle [NGSA]
    "Снятие юнитов с радара", // 77 Снятие юнитов с радара
    "/members", // 78 /members
    "Редактирование объявлений", // 79 Редактирование объявлений [CNN]
    "Использование алкотестера", // 80 Использование алкотестера [POLICE/ASGH]
    "/zachistka", // 81 Зачистка
    "Настройка наград в монетах" // 82 Монеты Континенталь
};
...
stock IsAFunctionOrganization(accessId, g, playerid) // Права доступа команды или функции на организацию
{
    ...
    else if(accessId == 35) // Установка собственных радаров // 35
    {
        if(IsPolice(g))  return 1;
    }
    else if(accessId == 36) // Удаление чужих радаров // 36
    {
        if(IsPolice(g))  return 1;
    }
    ...
    else if(accessId == 39) // /u /ub /d /db", // 39
    {
        if(IsADepart(playerid) || IsGangMember(playerid) || IsMafiaMember(playerid))  return 1;
    }
    ...
    {
        if (accessId <= 54) return 0; // Вырезанная команда /appoint, /suspend, /free
    ...
    else if(accessId >= 67 && accessId <= 69 || accessId == 71)
    {
    ...
    else if(accessId == 74) // 74 /boom [NGSA]
    {
        if(g == 3)  return 1;
    }
    else if(accessId == 75) // 75 Доставка мед оборудования [ASGH]
    {
        if(g == 4)  return 1;
    }
    else if(accessId == 76) // 76 /obstacle [NGSA]
    {
        if(g == 3) return 1;
    }
    else if(accessId == 77) // 77 Снятие юнитов с радара
    {
        if (IsPolice(g)) return 1;
    }
    else if(accessId == 79) // /adedit [CNN] 
    {
        if(g == 9)  return 1;
    }
    ...
    else if(accessId == 81) // /zachistka
    {
        if(IsAPoliceFunction(g) || g == 3) return 1;
    }
    else if(accessId == 82) // Монеты Континенталь
    {
        if(IsAMafiaID(g)) return 1;
    }
    else return 1;
    return 0;
}
...
stock SaveOrganAccess(idx, accid)
{
	...
	mysql_format(pearsq, string_mysql, sizeof(string_mysql), "UPDATE `pp_organization` SET `acc%d`='%d', `accdiv%d`='%d' WHERE `frakid`='%d'", 
