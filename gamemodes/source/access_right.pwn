...
new accessRightName[][] = // Команды и настройки в организации
{
    ...
    "/izol /unizol", // 72 посадить в изолятор, вытащить из изолятора [Police]
    "/kam" // 73 пересадить заключённого в другую камеру [Police]
};
...
stock IsAFunctionOrganization(accessId, g, playerid) // Права доступа команды или функции на организацию
{
    ...
    else if(accessId == 72) // 72 /izol /unizol
    {
        if(IsAPolice(g))  return 1;
    }
    else if(accessId == 73) // 72 /kam
    {
        if(IsAPolice(g))  return 1;
    }
    else return 1;
