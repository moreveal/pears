...
stock BuildTextString(type, id, obid, string:output[])
{
    new objectid;
    if(type == 1) objectid = DomInfo[id][dObject][obid];
    else if(type == 2) objectid = BizzInfo[id][bObject][obid];
    ...
    new yestext = GetDynamicObjectMaterialText(objectid, 0, text, fontsize, fontface, textfontsize, bold, fontcolor, backcolor, textalignment);
    if(yestext) 
    {
        format(output, 600, ", `ousetext` = '1', `oFontFace` = '%s', `oFontSize` = '%d', `oFontBold` = '%d',\
            `oFontColor` = '%d', `oBackColor` = '%d', `oAlignment` = '%d', `oTextFontSize` = '%d', `oObjectText` = '%s'"
            ,fontface, fontsize, bold, fontcolor, backcolor, textalignment, textfontsize, text);
    }
    else format(output, 64, ", `ousetext` = '0'");
    return 1;
}
