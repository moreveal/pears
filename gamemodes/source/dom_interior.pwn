...
function LoadObject(stat, typeLoad, owner_type) // Грузим объекты интерьера для дома и бизнеса
{
    ...
    for(new f = 0; f < rows; ++f)
    {
        ...
        if(owner_type == 0)
        {
            ...
            else if(typeLoad == 1) temptextures = NewLoadObject(f, 0); // 0 - Грузим дома
        }
        else if(owner_type == 1)
        {
            temptextures = NewLoadObject(f, 1); // 1 - Грузим Бизы
        }
...
// Новое сохранение объектов домов и бизов
function Call_InsertObjectOwner(nd, sla, owner_type)
{
    // if(owner_type == 0) DomInfo[nd][dNewid][sla] = cache_insert_id();
    // else if(owner_type == 1) BizzInfo[nd][bNewid][sla] = cache_insert_id();
    return 1;
}
...
stock UpdateObjectOwner(nd, sla, owner_type)
{
    ...
    if (JSON_Stringify(node, string_json) == JSON_CALL_NO_ERR) 
    {
        ...
        mysql_tquery(pearsq, string_mysql, "Call_InsertObjectOwner", "ddd", nd, sla, owner_type);
    }
    return 1;
}
