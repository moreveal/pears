...
stock CopyMaterialObjectDom(playerid, dom, oba)
{
    if(CheckObjectRedaktDom(playerid, dom, oba)) return true;
    ...
    new quan = CopyMaterialsFromObject(playerid, DomInfo[dom][dObject][oba]);
...
stock PasteMaterialObjectDom(playerid, dom, oba)
{
    if(CheckObjectRedaktDom(playerid, dom, oba)) return true;
...
stock PosObjectDom(playerid, dom, oba, Float:x = 0.0, Float:y = 0.0, Float:z = 0.0, Float:rx = 0.0, Float:ry = 0.0, Float:rz = 0.0)
{
    ...
    new Float:rob[3];
    GetDynamicObjectRot(DomInfo[dom][dObject][oba], rob[0], rob[1], rob[2]);
	...
	SetDynamicObjectPos(DomInfo[dom][dObject][oba], ob[0], ob[1], ob[2]);
    SetDynamicObjectRot(DomInfo[dom][dObject][oba], rob[0], rob[1], rob[2]);

    Update3DLabelDomBiz(dom, oba, 0);
    UpdateObject(dom, oba);
	return 1;
}

stock CheckObjectRedaktDom(playerid, dom, oba)
{
...
stock SetDomThisInterior(dom, intid)
{
    if(dom <= 0 || dom >= MAX_DOM) return false;
    DomInfo[dom][dInteriorPack] = intid;

    new string_mysql[100];
	mysql_format(pearsq, string_mysql, sizeof(string_mysql), "UPDATE `pp_dom` SET `dInteriorPack` = '%d' WHERE `Ids` = '%d'", DomInfo[dom][dInteriorPack], dom);
	query_empty(pearsq, string_mysql);
    return true;
}

// Проверка прав доступа на редактирование интерьера в доме
stock AccessRightDomInterior(playerid, nd)
{
	if(PlayerInfo[playerid][pDom] != nd)
	{
		if(DomInfo[nd][dAccfur] == 0) return ErrorMessage(playerid, "{FF6347}Изменять интерьер в этом доме может только владелец");
		if(DomInfo[nd][dAccfur] == 1 && PlayerInfo[playerid][pHouserent] != nd) return ErrorMessage(playerid, "{FF6347}Изменять интерьер в этом доме могут только владелец и проживающие");
		if(DomInfo[nd][dAccfur] == 2 && (DomInfo[nd][dFam] >= 1 && PlayerInfo[playerid][pFamily] != DomInfo[nd][dFam])) return ErrorMessage(playerid, "{FF6347}Изменять интерьер в этом доме могут только владелец и семья, которая здесь живёт");
