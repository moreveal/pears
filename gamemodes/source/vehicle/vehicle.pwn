...
#define MAX_VEHICLE_STOCK 212 // Стандартный транспорт
...
new VehCaseOff[MAX_VEHICLE_ALL]; // Статус доступности транспорта в обычном кейсе
...
stock SettingGosPriceVehicle(playerid, vehicleList)
{
	...
	if(VehSale[vehicleList]) format(line,sizeof(line),"\n{cccccc}Доступ к продаже и в кейсе:\t{99ff66}[ On ]"), strcat(lines,line);
	else format(line,sizeof(line),"\n{cccccc}Доступ к продаже и в кейсе:\t{FF6347}[ Off ]"), strcat(lines,line);

	if(VehCaseOff[vehicleList] == 1) format(line,sizeof(line),"\n{cccccc}Заблокировн в авто шкатулке:\t{99ff66}[ Да ]"), strcat(lines,line);
	else format(line,sizeof(line),"\n{cccccc}Заблокировн в авто шкатулке:\t{FF6347}[ Нет ]"), strcat(lines,line);
	...
	else if(dialogid == 1086)
	{
		if(response)
		{
			...
			else if(listitem == 2)
			{
				...
				format(line,sizeof(line),"\n{ffcc66}Примеры: Шкатулки, Контейнеры"), strcat(lines,line);
			...
			else if(listitem == 3)
			{
				...
				new log_str[128];
				new model = CorrectVehicleList(vehicleList);
			...
			else if(listitem == 4)
			{
				...
				if(VehCaseOff[vehicleList]) VehCaseOff[vehicleList] = 0;
				else VehCaseOff[vehicleList] = 1;
				SaveVehicleCaseOff(vehicleList);
				SettingGosPriceVehicle(playerid, OnlineInfo[playerid][oDialogMenu][3]);
			}
		}
...
// Сохраняем статус блокировки транспорта выпадения из авто шкатулки
stock SaveVehicleCaseOff(v)
{
    new string_mysql[140];
	mysql_format(pearsq, string_mysql, sizeof(string_mysql), "UPDATE `pp_priceveh` SET `VehCaseOff` = '%d' WHERE `model` = '%d'", VehCaseOff[v], v);
	query_empty(pearsq, string_mysql);
    return 1;
}

// Сохраняем лимитированное количествов транспорта в Шкатулку (до тех пор, пока игрок не распакует)
...
function LoadPriceVeh()
{
	...
	if(rows)
	{
		...
		for(new v = 0; v < MAX_VEHICLE_ALL; v++)
		{
			...
			cache_get_value_name_int(v, "VehCaseOff", VehCaseOff[vehicleList]);
			...
			{
				if(VehLimitedCase[vehicleList] >= VehLimited[vehicleList] // Если лимитированный транспорт в Шкатулках больше или равняется лимиту транспорта
					&& VehQuan[vehicleList] < VehLimitedCase[vehicleList]) // Если количество на руках меньше чем количество в Шкатулке
				{
		...
		// Собираем набор транспорта для Шкатулки
		CreateVehicleGiftCase();
