...
stock QuestActorJone(playerid) // Начинаем взаимодействовать с NPC квеста
{
    if(IsPlayerInRangeOfPoint(playerid,1.5, 1364.35242, -1682.73926, 13.47850) || IsPlayerInRangeOfPoint(playerid,1.5, 2121.7776,2709.5793,10.8203))
	{
        ...
        else if(OnlineInfo[playerid][oInHandThing][0] == 196) // Принесли пакет (Квест выполнен)
        {
            ...
            if(IsPlayerInRangeOfPoint(playerid,5.0, 1364.35242, -1682.73926, 13.47850)) GiveCar(playerid, freeSlot, QuestInfo[playerid][VehModel], 1362.8092,-1658.9130,13.1072,269.5840, 2, QuestInfo[playerid][VehColorQuest], QuestInfo[playerid][VehColorQuest], yesLoad, GetPlayerVirtualWorld(playerid), GetPlayerInterior(playerid));
            else if(IsPlayerInRangeOfPoint(playerid,5.0, 2121.7776,2709.5793,10.8203)) GiveCar(playerid, freeSlot, QuestInfo[playerid][VehModel], 2118.9817,2729.5417,10.5447,270.2156, 2, QuestInfo[playerid][VehColorQuest], QuestInfo[playerid][VehColorQuest], yesLoad, GetPlayerVirtualWorld(playerid), GetPlayerInterior(playerid));
...
stock OpenStartQuest(playerid, zoneid) // Запускаем зону квеста
{
    ...
    if(PursuitTime[playerid] >= 1) 
    {
        ErrorText(playerid, "{FF6347}Вы не можете пройти сейчас этот квест\n{cccccc}Вас преследует полиция");
        return 0;
    }
...
stock SaveQuest(playerid) // Сохраняем информацию о квестах
{
    ...
    mysql_format(pearsq, string_mysql, sizeof(string_mysql), "UPDATE `pp_igroki` SET `Quest`= '%e' WHERE `user_id`='%d'", part, PlayerInfo[playerid][pID]);
