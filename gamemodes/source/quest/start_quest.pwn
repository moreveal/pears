...
stock QuestActorJoneSeks(playerid)
{
    ...
    else if(PlayerInfo[playerid][pQuest][6] == 3)
    {
        ...
        SetPVarInt(playerid,"qweststat",22), SetPVarInt(playerid,"qwesttime",8);
    }
    SaveQuest(playerid);
    return 1;
}
...
stock StartFirstCallQuest(playerid)
{
    ...
    else if(NoCompleteQuest(playerid, 5)) ShowQwest(playerid, 5); // Исследование гробницы
    else if(NoCompleteQuest(playerid, 6)) ShowQwest(playerid, 6); // Отдых в клубе
    return 1;
}
