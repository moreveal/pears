...
stock QuestActorBruce(playerid) // Начинаем взаимодействовать с NPC квеста Археалогия
{
    ...
    {
        //PlayAudioStreamForPlayer(playerid, "https://cdn.pears.fun/sound/characters/bruce/bruce6.mp3"); // Перезаписать новую фразу
...
stock QuestActorJoneSeks(playerid)
{
    ...
    if(PlayerInfo[playerid][pQuest][6] == 1)
    {
        ...
        if(BizzInfo[b][bShluha] == 0) return ErrorMessage(playerid,"{ff6347}В данном клубе к сожалению нет проститутки, нужно поехать и найти клуб с проституткой!\n\nДля этого [ Y > GPS > Развлечениея > Клубы]");
...
stock QuestActorJoneNotebook(playerid) // Начинаем взаимодействовать с NPC квеста Археалогия
{
    ...
    if(PursuitTime[playerid] >= 1) return ErrorMessage(playerid, "{FF6347}Вы не можете пройти сейчас этот квест\n{cccccc}Вас преследует полиция");
    if(QuestInfo[playerid][ScriptQuest] == 3) return 1; // Все сценарии были отработаны

    if(PlayerInfo[playerid][pQuest][9] == 1)
    {
        SuccessMessage(playerid,"{44ff99}Подойдите к кассе и купите ноутбук");
...
stock StartFirstCallQuest(playerid)
{
    ...
    {
        if(NoCompleteQuest(playerid, 5)) ShowQwest(playerid, 5); // Археолог
    }
    else
    {
        if(NoCompleteQuest(playerid, 3)) ShowQwest(playerid, 3); // Джоне (Взлом транспорта)
        else ShowQwest(playerid, 5); // Археолог
    }
    return 1;
}
