...
{
    if(IsPolice(fraction(playerid)) && PlayerInfo[playerid][pJailed] == 0) // Если мы мент и не в заключении, просто можем переключать дверь
    {
...
stock PrisonMovingPoster(playerid,number)
{
    ...
    if(IsPoliceMember(playerid) && PlayerInfo[playerid][pJailed] == 0)
    {
...
stock pt_limit(pt, thingId, &getQuan, &getLimit) // Проверяем лимиты тумбы в тюрьме
{
    ...
    lim[271] = lim[272] = lim[277] = lim[278] = 50; // Упаковки веществ
