...
stock item_boot(playerid, v, fpick, fquan, inva, fpara, thingType, thingPack)
{
	...
	if(fpick == 0)
	{
		...
		else
		{
   			if(IsA_Gen5(v))
		 	{
		 		if(inva <= 24) PlayerTextDrawBackgroundColour(playerid, PlaNestPick[inva][playerid], PlayerInfo[playerid][pStyle1]), PlayerTextDrawColour(playerid, PlaNestPick[inva][playerid], PlayerInfo[playerid][pStyle1]);
  				else blocker = 1;
   			}
		 	else if(IsA_Gen10(v))
		 	{
		 		if(inva <= 29) PlayerTextDrawBackgroundColour(playerid, PlaNestPick[inva][playerid], PlayerInfo[playerid][pStyle1]), PlayerTextDrawColour(playerid, PlaNestPick[inva][playerid], PlayerInfo[playerid][pStyle1]);
		 		else blocker = 1;
		 	}
		 	else if(IsA_Gen15(v))
		 	{
		 		if(inva <= 34) PlayerTextDrawBackgroundColour(playerid, PlaNestPick[inva][playerid], PlayerInfo[playerid][pStyle1]), PlayerTextDrawColour(playerid, PlaNestPick[inva][playerid], PlayerInfo[playerid][pStyle1]);
		 		else blocker = 1;
		 	}
		 	else PlayerTextDrawBackgroundColour(playerid, PlaNestPick[inva][playerid], PlayerInfo[playerid][pStyle1]), PlayerTextDrawColour(playerid, PlaNestPick[inva][playerid], PlayerInfo[playerid][pStyle1]);
		}
		if(blocker == 1)
		{
...
stock v_limit(v, thingId, &getQuan, &getLimit) // Проверяем лимиты дома
{
	...
	lim[4] = 10000, lim[5] = 10000, lim[6] = 10000, lim[9] = 20, lim[18] = 1000, lim[20] = 10000, lim[27] = 10000, lim[28] = 10000, lim[29] = 10000, lim[30] = 10000;
	lim[22] = 100; // Мясо
	...
	lim[271] = lim[272] = lim[277] = lim[278] = 2000; // Упаковки веществ
	lim[279] = 1000; // Резиновая смесь
...
{
	if (stat == 35) return VehInfo[v][vPeople] - 1;
...
stock get_boot_para(v, stat) // Параметр предмета
{
	new para = 0;
	for(new inva = 0; inva < 20; inva++)
	{
