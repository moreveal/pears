...
CMD:testcar(playerid) // VREMENNO
{
	...
	showMenu_VehicleShop(playerid, 77);
	return 1;
}
...
stock showMenu_VehicleShop(playerid, bizId) // Открываем меню автосалона
{
    ...
    TP[1][playerid] = 0; // Транспорт в слоте 0
...
stock createDraw_VehicleShop(playerid) // Создаём текстдравы
{
    ...
    PlayerTextDrawColor(playerid, VehicleShopDraw[17][playerid], -1);
